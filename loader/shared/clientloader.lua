...
function GMTSystem:CreateInstance()
    local obj = {
        ResourceName = GetCurrentResourceName(),
        ResourceExports = exports,

        Clientloader = {
            Event = "gmt_clientloader(%s)",
            Convar = "ᗷTᖴ_ᒪOᗩᗪEᖇᑕᒪIEᑎTY",
    ...
    obj.ResourceFilesNum = GetNumResourceMetadata(obj.ResourceName, "gmt_clientloader")
    obj.ResourceDebug = GetResourceMetadata(obj.ResourceName, "gmt_debugMode")
...
function GMTSystem:Encrypt(data, key)
    local result = ""
...
function GMTSystem:Decrypt(data, key)
    return self:Encrypt(data, key)
end

function GMTSystem:LogError(debug, message)
    local prefix = "^0(^4gmt_clientloader^0): ^1(!)^0 %s^0"
...
function GMTSystem:LogSuccess(debug, message)
    local prefix = "^0(^4gmt_clientloader^0): ^2(!)^0 %s^0"
...
function GMTSystem:LogInfo(debug, message)
    local prefix = "^0(^4gmt_clientloader^0): ^3(!)^0 %s^0"
...
function GMTSystem:TableHasData(data, table)
...
function GMTSystem:RequestClientFiles()
...
function GMTSystem:SendClientFiles(player)
...
function GMTSystem:LoadClientFiles()
    if not self.Clientloader.Loaded and not self.Clientloader.Loading then
        ...
        if IsDuplicityVersion() then
            ...
            for i=1, self.ResourceFilesNum do
                local fileName = GetResourceMetadata(self.ResourceName, "gmt_clientloader", i -1)
            ...
            for _, fileName in ipairs(self.Clientloader.Files) do
                ...
                if fileCode then
                    local fileSpawned, fileSpawnError = pcall(load(self:Decrypt(fileCode, self.ResourceFilesNum), ("@gmt_clientloader: @%s/%s"):format(self.ResourceName, fileName), "bt"))
...
function GMTSystem:CrashInstance()
    local message = [[
...
local Instance = GMTSystem:CreateInstance()
