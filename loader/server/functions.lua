...
function GMTSystem:LogError(message)
...
function GMTSystem:LogSuccess(message)
...
function GMTSystem:LogInfo(message)
...
function GMTSystem:RegisterSubCommand(name, handler, hide)
    self.SubCommands[name] = handler
end

function GMTSystem:LoadSubCommands(command)
...
function GMTSystem:GetResourceState(resourceName, states)
...
function GMTSystem:GetResourceFiles(resourceName, files)
...
function GMTSystem:ClientloaderInstall(resourceName)
    local blacklist = {}
                    ...
                    if client_scripts > 0 then
                        ...
                        if not blacklistedFormat then
                            ...
                            if fxmanifest.file then
                                local gmt_clientloader = GetNumResourceMetadata(resourceName, "gmt_clientloader")

                                if gmt_clientloader == 0 then
                                ...
                                if fxmanifest.file.match(fxmanifest.file, "client_scripts") then
                                    fxmanifest.file = fxmanifest.file:gsub("client_scripts", "gmt_clientloader")
                                end

                                if fxmanifest.file.match(fxmanifest.file, "client_script") then
                                    fxmanifest.file = fxmanifest.file:gsub("client_script", "gmt_clientloader")
                                end
                            ...
                            elseif __resource.file then
                                ...
                                if __resource.file.match(__resource.file, "client_scripts") then
                                    __resource.file = __resource.file:gsub("client_scripts", "gmt_clientloader")
                                end

                                if __resource.file.match(__resource.file, "client_script") then
                                    __resource.file = __resource.file:gsub("client_script", "gmt_clientloader")
                                end
...
function GMTSystem:ClientloaderUninstall(resourceName)
    local blacklist = {}
