...
local function routeMessage(source, author, message, mode, fromConsole)
    if source >= 1 then
        author = function GMT.GetDiscordName(user_id)
    end
    ...
    if mode == "OOC" then
        TriggerEvent("MG:ooc", source, message)
        return
    end
    if mode == "Gang" then 
        TriggerEvent("MG:GangChat", source, message)
        return
    end
    if mode == "Anonymous" then
        TriggerEvent("MG:Anon", source, message)
        return
    end
    if mode == "Police" then
        TriggerEvent("MG:PoliceChat", source, message)
        return
    end
    if mode == "NHS" then
        TriggerEvent("MG:Nchat", source, message)
        return
    end
    if mode == "Admin" then
        for k,v in ipairs(GetPlayers()) do
            if exports['mg']:hasPermission(v, "admin.tickets") then
...
AddEventHandler('_chat:messageEntered', function(author, color, message, mode)
    ...
    if not WasEventCanceled() then
        if cooldown[source] and not (os.time() > cooldown[source]) then
            TriggerClientEvent('chatMessage', source, "MG",  { 255, 255, 255 }, "You are being rate limited.", "alert", mode)
            return
        else
            cooldown[source] = nil
        end
		...
		for word in pairs(blockedWords) do
			if(string.gsub(string.gsub(string.gsub(string.gsub(string.gsub(string.gsub(message:lower(), "-", ""), ",", ""), "%.", ""), " ", ""), "*", ""), "+", ""):find(blockedWords[word])) then
				TriggerClientEvent('MG:chatFilterScaleform', source, 10, 'That word is not allowed.')
				CancelEvent()
				return
			end
		end
...
AddEventHandler('__cfx_internal:commandFallback', function(command)
    local name = function GMT.GetDiscordName(user_id)
