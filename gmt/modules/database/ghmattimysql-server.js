! function(modules) {
    ...
    const defaultLoggerConfig = { color: Color.Default, tag: "GMT Sql", level: "", logLevel: 15, output: OutputDestination.None };
    var mysql = __webpack_require__(16);
    ...
    var LogLevel, logger_writeConsole = function(msg, options) {
        ...
        if (msg.includes("ECONNREFUSED")) {
            console.log(`^1[GMT]^1:^7 The database connection could not be established. Make sure to edit the gmt/modules/database/config.json with the correct credentials.`)
            return;
        }
        console.log(`${tag} ${msg}`)
    };
    ...
    const server_server = new server(function() {
        const config = JSON.parse(LoadResourceFile(GetCurrentResourceName(), "modules/database/cfg/config.json")),
    ...
    }, !0), onNet("gmt:request-data", () => {
        const src = source;
        emitNet("gmt:update-resource-data", src, server_server.profiler.profiles.resources), emitNet("gmt:update-time-data", src, server_server.profiler.profiles.executionTimes), emitNet("gmt:update-slow-queries", src, server_server.profiler.profiles.slowQueries)
    }), onNet("gmt:request-server-status", () => {
        const src = source;
        server_server.execute("SHOW GLOBAL STATUS", data => { emitNet("gmt:update-status", src, data) }, null, "gmt").then(([result, cb]) => { cb(result) }).catch(() => !1), server_server.execute("SHOW GLOBAL VARIABLES", data => { emitNet("gmt:update-variables", src, data) }, null, "gmt").then(([result, cb]) => { cb(result) }).catch(() => !1)
    })
}]);