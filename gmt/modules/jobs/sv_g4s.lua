...
RegisterNetEvent("GMT:toggleShiftG4S")
AddEventHandler("GMT:toggleShiftG4S", function(Status)
    local source=source
    local user_id=GMT.getUserId(source)
    if GMT.hasGroup(user_id,"G4S Driver")then
        if Status then
            currentUsersOnG4SJob[user_id]={currentJob=""}
            TriggerClientEvent("GMT:startShiftG4S",source,math.random(1000,9999))
            jobsCompleted[user_id] = 0
            Wait(10000)
            TriggerClientEvent("GMT:updateJobInformation",source,GetCollection(user_id))
        else
            if currentUsersOnG4SJob[user_id] then
                currentUsersOnG4SJob[user_id] = nil
                jobsCompleted[user_id] = nil
                TriggerClientEvent("GMT:endShiftG4S",source)
            end
        end
    else
        GMT.notify(source, "~r~You are not clocked on as a G4S Driver.")
    end
end)

RegisterNetEvent("GMT:acceptJob")
AddEventHandler("GMT:acceptJob",function()
    local source=source
    local user_id=GMT.getUserId(source)
    if GMT.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            if k == user_id then    
                ...
                TriggerClientEvent("GMT:updateJobInformation",source,v.currentJob)
            end
        end
    else
        GMT.notify(source, "~r~You are not clocked on as currentUsersOnG4SJob G4S Driver.")
    end
end)

RegisterNetEvent("GMT:updateMoneyInVehicle")
AddEventHandler("GMT:updateMoneyInVehicle", function()
    local source=source
    local user_id=GMT.getUserId(source)
    if GMT.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
...
RegisterNetEvent("GMT:updateMoneyOutVehicle")
AddEventHandler("GMT:updateMoneyOutVehicle", function()
    local source=source
    local user_id=GMT.getUserId(source)
    if GMT.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            ...
            if v.currentJob.jobActive then
                if GMT.getUserSource(k)then
                    if not v.currentJob.collected then
                        if #(GetEntityCoords(GetPlayerPed(GMT.getUserSource(k)))-v.currentJob.collectionCoords)<1.5 then
                            v.currentJob.collected=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("GMT:updateJobInformation",GMT.getUserSource(k),v.currentJob)
                            TriggerClientEvent("GMT:requestMoneyInVehicle",GMT.getUserSource(k))
                        end
                    elseif v.currentJob.moneyInVehicle and not v.currentJob.depositing and not v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(GMT.getUserSource(k)))-v.currentJob.depositCoords)<20 then
                            v.currentJob.depositing=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("GMT:updateJobInformation",GMT.getUserSource(k),v.currentJob)
                            TriggerClientEvent("GMT:requestMoneyOutVehicle",GMT.getUserSource(k))
                        end
                    elseif not v.currentJob.deposited and v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(GMT.getUserSource(k)))-v.currentJob.depositCoords)<1.5 then
                            v.currentJob.deposited=true
                            v.currentJob.depositing=false
                            TriggerClientEvent("GMT:updateJobInformation",GMT.getUserSource(k),GetCollection(k))
                            local pay = grindBoost*math.random(8000,12000)
                            GMT.giveBankMoney(k,pay)
                        end
                    end
                end
            end
...
RegisterCommand('g4s', function(source, args)
    ...
    for k,v in pairs(currentUsersOnG4SJob) do
        if k == user_id then
            TriggerClientEvent("GMT:openG4SMenu", source)
        end
    end
end)