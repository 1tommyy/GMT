...
RegisterNetEvent("GMT:attemptBeginRoyalMailJob")
AddEventHandler("GMT:attemptBeginRoyalMailJob",function()
    local source=source
    local user_id=GMT.getUserId(source)
    local RoyalMailJobTable={}
    if GMT.hasGroup(user_id,"Royal Mail Driver")then
        if not usersInRoyalMailJob[user_id] then
            ...
            TriggerClientEvent("GMT:beginRoyalMailJob",source)
            Wait(100)
            TriggerClientEvent("GMT:royalMailJobSetNextBlip",source,RoyalMailDrops[RoyalMailJobTable.stopNumber].dropPosition)
        else
            GMT.notify(source, "~r~You are already a Royal Mail Driver.")
        end
    else
        GMT.notify(source, "~r~You are not clocked on as a Royal Mail Driver.")
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(RoyalMailDrops)do
                    if GMT.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(GMT.getUserSource(k)))-RoyalMailDrops[v.currentJob.stopNumber].dropPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            GMT.giveBankMoney(k,pay)
                            TriggerClientEvent("GMT:royalMailReachedNextStop",GMT.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition,pay)
                            if v.currentJob.stopNumber==#RoyalMailDrops then
                                TriggerClientEvent("GMT:royalMailJobEnd", GMT.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInRoyalMailJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("GMT:royalMailJobSetNextBlip",GMT.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition)
                            end
                        end
                    end
                end
