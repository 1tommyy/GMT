...
RegisterNetEvent("GMT:attemptBeginDeliverooJob")
AddEventHandler("GMT:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=GMT.getUserId(source)
    local deliverooJobTable={}
    if GMT.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("GMT:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("GMT:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            GMT.notify(source, "You are already a Deliveroo Driver.")
        end
    else
        GMT.notify(source, "~r~You are not clocked on as a Deliveroo Driver.")
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(deliverooStops)do
                    if GMT.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(GMT.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            GMT.giveBankMoney(k,pay)
                            TriggerClientEvent("GMT:deliverooJobReachedNextStop",GMT.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#deliverooStops then
                                TriggerClientEvent("GMT:deliverooJobEnd", GMT.getUserSource(k), true)
                                v.currentJob.jobActive=false
                                usersInDeliverooJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("GMT:deliverooJobSetNextBlip",GMT.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
