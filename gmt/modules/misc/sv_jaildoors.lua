...
AddEventHandler("GMT:onServerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('GMT:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("GMT:policeSetDoorState")
AddEventHandler("GMT:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = GMT.getUserId(source)
    if GMT.hasPermission(user_id, 'police.armoury') then
        doorStates[doorHash] = state
        TriggerClientEvent('GMT:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("GMT:enterPoliceSyncDoors")
AddEventHandler("GMT:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('GMT:gotAllDoorStates', source, doorStates)
end)