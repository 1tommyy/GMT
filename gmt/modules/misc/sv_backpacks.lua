            ...
            GMT.getSubscriptions(user_id, function(cb, plushours, plathours)
                if cb then
                    ...
                    if data.invcap > invcap then
                        GMT.notify(source, '~r~You cannot use this item as you have a backpack already!') return
                    end
                    if GMT.tryPayment(user_id, b[4]) then
                        ...
                        GMT.notifyPicture6(source, "walletnotification", "notification", "~g~" .. backpackname .. " ~s~Purchased.", "Wallet", "Backpack purchase")
                    else
    ...
    for k,v in pairs(cfg.stores) do
        if backpackstorename == 'Rebel' and k == 'Rebel' then
            if GMT.hasPermission(user_id, 'rebellicense.whitelisted') then
                ...
                GMT.notify(source, '~r~You do not have permissions to purchase from this store.')
            end
