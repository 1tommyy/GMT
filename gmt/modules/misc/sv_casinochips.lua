...
AddEventHandler("playerJoining", function()
    ...
    exports["gmt"]:executeSync("INSERT IGNORE INTO gmt_casino_chips SET user_id = @user_id", {user_id = user_id})
end)
...
AddEventHandler("GMT:buyChips", function(amount)
    ...
    if GMT.tryPayment(user_id, amount) then
        ...
        GMT.notifyPicture6(source, "walletnotification", "notification", "Charged ~r~£" .. getMoneyStringFormatted(amount) .. " ~s~from your account.", "Wallet", "Casino purchase")
        GMT.sendDCLog('purchase-chips',"GMT Chip Logs", "> Player Name: **"..GMT.getPlayerName(user_id).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Amount: **"..getMoneyStringFormatted(amount).."**")
        return
    else
        ...
        MySQL.query("casinochips/get_chips", {user_id = user_id}, function(rows, affected)
            if #rows > 0 then
                ...
                if amount > 0 and chips > 0 and chips >= amount then
                    ...
                    TriggerClientEvent('GMT:chipsUpdated', source)
                    GMT.sendDCLog('sell-chips',"GMT Chip Logs", "> Player Name: **"..GMT.getPlayerName(user_id).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Amount: **"..getMoneyStringFormatted(amount).."**")
                    GMT.giveMoney(user_id, amount)
                else
                    GMT.notify(source, "You don't have enough chips.")
                end
