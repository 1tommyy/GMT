function rank(_, arg)
    ...
    if GMT.getUserSource(user_id) then
        GMT.addUserGroup(user_id,rank)  
        TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received "..rank.." rank we appreciate your support!\n", 18, 10000)
    else
        exports['gmt']:execute("SELECT * FROM gmt_user_data WHERE user_id = @user_id", {user_id = user_id}, function(result) 
            if #result > 0 then
                local dvalue = json.decode(result[1].dvalue)
                local groups = dvalue.groups
                groups[rank] = true
                exports['gmt']:execute("UPDATE gmt_user_data SET dvalue = @dvalue WHERE user_id = @user_id", {dvalue = json.encode(dvalue), user_id = user_id}, function() end)
            end
        end)
    end  
...
function moneybag(_, arg)
    ...
    if GMT.getUserSource(user_id) then
        ...
        TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received a £"..amount.." money bag we appreciate your support!\n", 18, 10000)
    else
        exports['gmt']:execute("UPDATE gmt_user_moneys SET bank = bank + @amount WHERE user_id = @user_id", {amount = amount, user_id = user_id}, function() end)
    end
...
function plus(_, arg)
    ...
    GMT.getSubscriptions(user_id, function(cb, plushours, plathours)
        TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received GMT Plus we appreciate your support!\n", 18, 10000)
        if cb then
...
function platinum(_, arg)
    ...
    local newhours = tonumber(arg[2])
    GMT.getSubscriptions(user_id, function(cb, plushours, plathours)
        TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received GMT Platinum we appreciate your support!\n", 18, 10000)
        if cb then
...
function addweaponwhitelist(_, arg)
    if _ ~= 0 then return end -- tayser
    local user_id = tonumber(arg[1])
    local code = tonumber(arg[2])
    local ownedWhitelists = {}
    MySQL.query("GMT/get_weapon_codes", {}, function(weaponCodes)
        TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received your weapon whitelist we appreciate your support!\n", 6, 10000)
            ...
            for e,f in pairs(weaponCodes) do
                if f['user_id'] == user_id and f['weapon_code'] == code then
                    MySQL.query("GMT/get_weapons", {user_id = user_id}, function(weaponWhitelists)
                        ...
                        MySQL.execute("GMT/remove_weapon_code", {weapon_code = code})
...
function setphonenumber(_, arg)
    ...
    MySQL.query("GMT/get_userbyphone", {phone_number}, function(phoneNumberTaken)
        TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have changed your phone number to "..phone_number.." we appreciate your support!\n", 6, 10000)
        if #phoneNumberTaken > 0 then
        else
            MySQL.execute("GMT/update_user_phone", {phone = phone_number, user_id = user_id})
...
function vipcar(_, arg)
    ...
    TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received a "..spawncode.." in your garage we appreciate your support!\n", 18, 10000)
    MySQL.execute("GMT/add_vehicle", {user_id = user_id, vehicle = spawncode, registration = 'P'..math.random(10000,99999)})
...
function vipcar2(_, arg)
    ...
    TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received a "..spawncode.." in your garage we appreciate your support!\n", 18, 10000)
    MySQL.execute("GMT/add_vehicle", {user_id = user_id, vehicle = spawncode2, registration = 'P'..math.random(10000,99999)})
...
function vipcar3(_, arg)
    ...
    TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received a "..spawncode.." in your garage we appreciate your support!\n", 18, 10000)
    MySQL.execute("GMT/add_vehicle", {user_id = user_id, vehicle = spawncode3, registration = 'P'..math.random(10000,99999)})
...
function vipcar4(_, arg)
    ...
    TriggerClientEvent('GMT:smallAnnouncement', user_id, 'GMT Studios', "You have received a "..spawncode.." in your garage we appreciate your support!\n", 18, 10000)
    MySQL.execute("GMT/add_vehicle", {user_id = user_id, vehicle = spawncode4, registration = 'P'..math.random(10000,99999)})
