...
AddEventHandler("GMT:proposeCoinflip",function(betAmount)
    ...
    if betAmount then 
                ...
                if betAmount >= 100000 then
                    MySQL.query("casinochips/get_chips", {user_id = user_id}, function(rows, affected)
                        ...
                        if chips >= betAmount then
                            ...
                            GMT.notify(source, "~g~Bet placed: " .. getMoneyStringFormatted(betAmount) .. " chips.")
                        else 
       ...
       GMT.notify(source, "Error betting!")
    end
end)
            ...
            MySQL.query("casinochips/get_chips", {user_id = user_id}, function(rows, affected)
                ...
                if chips >= v.betAmount then
                    ...
                    if coinFlipOutcome == 0 then
                        local game = {amount = v.betAmount, winner = GMT.getPlayerName(user_id), loser = GMT.getPlayerName(GMT.getUserSource(v.user_id))}
                        ...
                        TriggerClientEvent('GMT:chipsUpdated', source)
                        GMT.sendDCLog('coinflip-bet',"GMT Coinflip Logs", "> Winner Name: **"..GMT.getPlayerName(user_id).."**\n> Winner TempID: **"..source.."**\n> Winner PermID: **"..user_id.."**\n> Loser Name: **"..GMT.getPlayerName(GMT.getUserSource(v.user_id)).."**\n> Loser TempID: **"..GMT.getUserSource(v.user_id).."**\n> Loser PermID: **"..v.user_id.."**\n> Amount: **"..getMoneyStringFormatted(v.betAmount).."**")
                    else
                        local game = {amount = v.betAmount, winner = GMT.getPlayerName(GMT.getUserSource(v.user_id)), loser = GMT.getPlayerName(user_id)}
                        ...
                        TriggerClientEvent('GMT:chipsUpdated', GMT.getUserSource(v.user_id))
                        GMT.sendDCLog('coinflip-bet',"GMT Coinflip Logs", "> Winner Name: **"..GMT.getPlayerName(GMT.getUserSource(v.user_id)).."**\n> Winner TempID: **"..GMT.getUserSource(v.user_id).."**\n> Winner PermID: **"..v.user_id.."**\n> Loser Name: **"..GMT.getPlayerName(user_id).."**\n> Loser TempID: **"..source.."**\n> Loser PermID: **"..user_id.."**\n> Amount: **"..getMoneyStringFormatted(v.betAmount).."**")
                    end
                else 
                    GMT.notify(source, "Not enough chips!")
                end
