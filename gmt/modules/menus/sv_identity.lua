...
AddEventHandler("GMT:ChangeIdentity", function(first, second, age)
    ...
    if user_id ~= nil then
        if GMT.tryBankPayment(user_id,5000) then
          TriggerClientEvent("GMT:gotCurrentIdentity", source, first, second, age)
...
RegisterServerEvent("GMT:showMyIdentity", function()
  local source = source
  local user_id = GMT.getUserId(source)
  GMT.getUserIdentity(user_id, function(identity)
    if identity then
      TriggerClientEvent('GMT:showIdentity', source, source, true, identity.firstname, identity.name, '19/01/1990', identity.phone, '10/02/2015', '10/02/2025', {})
      TriggerClientEvent('GMT:setNameFields', source, identity.name, identity.firstname)
      GMT.request(source, "Hide the ID card.", 1000, function(player,ok)
        TriggerClientEvent('GMT:hideIdentity', source)
      end)
    end
  end)
end)