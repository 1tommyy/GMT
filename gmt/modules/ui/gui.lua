...
function GMT.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  GMTclient.openMenuData(source,{menudata})
end

-- force close player menu
function GMT.closeMenu(source)
  GMTclient.closeMenu(source,{})
end
...
function GMT.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result
  GMTclient.prompt(source,{title,default_text})
end
...
function GMT.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  GMTclient.request(source,{id,text,time}) -- send request to client
...
function GMT.registerMenuBuilder(name, builder)
...
function GMT.buildMenu(name, data, cbr)
...
function GMT.openMainMenu(source)
  GMT.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    GMT.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tGMT.closeMenu(id)
...
function tGMT.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tGMT.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tGMT.requestResult(id,ok)
...
function tGMT.openMainMenu()
  GMT.openMainMenu(source)
end
...
function GMT.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for GMT.addStaticMenuChoices calls
...
AddEventHandler("GMT:onServerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    GMTclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
