...
function tGMT.getStreetNameAtCoord(v, w, H)
...
RegisterNetEvent("GMT:playClientNuiSound",function(aI, aJ, ap) -- use for cuffing and other sounds in areas
    local ax = tGMT.getPlayerCoords()
    if #(ax - aI) <= ap then
        SendNUIMessage({transactionType = aJ})
    end
end)
function tGMT.getNetId(aW, aX)
    if aX == nil then
        aX = ""
    end
    local aY = 0
...
function tGMT.getVehicleNameFromId(aZ)
    if aT[string.lower(aZ)] then
        return aT[string.lower(aZ)].name
    end
    return ""
end
function tGMT.getGarageNameFromId(aZ)
    return aT[string.lower(aZ)].garageType
end
function tGMT.getVehicleIdFromModel(s)
    return aU[s]
end
local a_ = math.rad
local b0 = math.cos
local b1 = math.sin
local b2 = math.abs
function tGMT.rotationToDirection(b3)
    local B = a_(b3.x)
    local D = a_(b3.z)
