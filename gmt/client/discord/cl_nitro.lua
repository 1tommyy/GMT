...
Citizen.CreateThread(
    function()
        while true do
            local n = GMT.getPlayerPed()
            local o = GetVehiclePedIsIn(GMT.getPlayerPed(), false)
            if o ~= 0 and GetPedInVehicleSeat(o, -1) == n then
                local p, q = GMT.getVehicleInfos(o)
                local r = DecorGetInt(o, "GMT_owner")
                if p == GMT.getUserId() then
                    if a[q] then
                        local s = a[q]
                        d = q
                        c = true
                        while GetVehiclePedIsIn(GMT.getPlayerPed(), false) ~= 0 do
                            Wait(1000)
                        end
                        c = false
                        offset = s - a[q]
                        TriggerServerEvent("GMT:updateNitro", q, -offset)
                    end
                end
            end
            Wait(0)
...
function func_drawNitroText()
    if c and a[d] > 0 then
        ...
        GMT.DrawText(l, m, "Nitro: " .. math.floor(tonumber(a4)) .. "%", k, j, 1, {f, g, h, i}, true)
        local o = GetVehiclePedIsIn(GMT.getPlayerPed(), false)
...
GMT.createThreadOnTick(func_drawNitroText)
