...
local function B()
    ...
    local D = 500 * math.ceil(#(GMT.getPlayerCoords() - GetEntityCoords(b)) / 10)
    if j then
...
RMenu.Add(
    "g4s",
    "shiftHandler",
    RageUI.CreateMenu("", "~s~S~w~ecurity Company", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight(), "menus", "g4sjob")
)
...
tGMT.addMarker(d.x, d.y, d.z, 1.2, 1.2, 1.2, 0, 255, 125, 125, 50, 0, true, true)
local F = tGMT.addBlip(d.x, d.y, d.z, 457, 18, "G4S Depot", 1.0, false)
local G = function()
...
GMT.createArea("g4sDepot_", d, 1.5, 6, G, H, I)
    ...
    function()
        while true do
            if not y.hackingEngaged and not a and not y.awaitingResponse then
                if table.count(s) > 0 and GMT.getPlayerVehicle() == 0 then
                    for K, L in pairs(s) do
                        ...
                        if table.has(p, O) or not DoesEntityExist(N) then
                            ...
                            if IsVehicleStopped(N) then
                                local P = #(GMT.getPlayerCoords() - GetEntityCoords(N))
                                ...
                                if P <= 11.0 then
                                    ...
                                    if IsControlJustPressed(1, 18) then
                                        ...
                                        y.entity = N
                                        y.src = O
                                        TriggerServerEvent("GMT:requestHackingChanceServer", M, O)
...
RegisterNetEvent(
    ...
    function(b, W, X, Y)
        if Y then
            ...
            local o = GMT.loadModel("prop_cs_tablet")
            v = CreateObject(o, 0, 0, 0, true, true, true)
            AttachEntityToEntity(
                v,
                GMT.getPlayerPed(),
                GetPedBoneIndex(GMT.getPlayerPed(), 57005),
                0.17,
                0.10,
                -0.13,
                24.0,
            ...
            if HasScaleformMovieLoaded(R) then
                ...
                if IsScaleformMovieMethodReturnValueReady(S) then
                    ...
                    elseif ProgramID == 87 then
                        ...
                        TriggerServerEvent("GMT:updateHackingOutcome", y.netId, y.src, U, false)
                        DeleteObject(v)
                        break
                    ...
                    elseif ProgramID == 86 then
                        ...
                        TriggerServerEvent("GMT:updateHackingOutcome", y.netId, y.src, U, true)
                        DeleteObject(v)
                        break
    ...
    function()
        if RageUI.Visible(RMenu:Get("g4s", "main")) then
            RageUI.DrawContent(
                ...
                function()
                    if not j then
                        if GMT.getPlayerVehicle() == 0 and w < 4 then
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(Z, _, a0)
                                if a0 then
                                    TriggerServerEvent("GMT:toggleShiftG4S", false)
                                    a = false
                                end
                            end
                        )
                        ...
                        if not h then
                            RageUI.ButtonWithStyle(
                                ...
                                function(Z, _, a0)
                                    if a0 then
                                        TriggerServerEvent("GMT:activatePanicServer", c)
                                        imageNotification(
                                            "notification_images",
                                            "g4slogo",
                    ...
                    if not a then
                        RageUI.ButtonWithStyle(
                            ...
                            function(Z, _, a0)
                                if a0 then
                                    RageUI.Visible(RMenu:Get("g4s", "shiftHandler"), false)
                                    TriggerServerEvent("GMT:toggleShiftG4S", true)
                                end
                            end
                        )
                    else
...
function newG4sVehicle()
    ...
    if DoesEntityExist(b) and not IsVehicleDriveable(b, true) then
        ...
        b = GMT.spawnVehicle(n, a1.x, a1.y, a1.z, GetEntityHeading(C), true, true)
        c = VehToNet(b)
        TriggerServerEvent("GMT:updateCurrentVehicle", c)
        w = w + 1
    end
end
function securityCheck(C)
    local a2 = GMT.getPlayerVehicle()
    if a2 ~= 0 and a2 and b and b ~= 0 then
        if a2 ~= b then
            ...
            SetTimeout(
                15000,
                function()
                    a2 = GetVehiclePedIsIn(GMT.getPlayerPed(), false)
                    if a2 ~= 0 then
                        if a2 ~= b then
                            TriggerServerEvent("GMT:toggleShiftG4S", false)
                            tGMT.removeBlip(k)
...
RegisterNetEvent(
    "GMT:resetPanicStatus",
    function()
        h = false
    end
)
RegisterNetEvent(
    "GMT:updateJobInformation",
    function(a3)
        if l ~= 0 then
            tGMT.removeBlip(l)
        end
        if a3.jobActive then
            ...
            Wait(3000)
            TriggerServerEvent("GMT:acceptJob")
        end
    end
)
...
RegisterNetEvent(
    ...
    function(a4)
        ...
        local J = GMT.getPlayerCoords()
        a = true
        GMT.disableCustomizationSave(a)
        b = GMT.spawnVehicle(n, z.x, z.y, z.z, z.heading, true, true)
        c = VehToNet(b)
        TaskWarpPedIntoVehicle(C, b, -1)
        g = a4
        local a5 = {"Jack", "SEAN", "Serio", "Tayser", "GMT"}
        ...
        x = json.encode(tGMT.getCustomization())
        if a8 then
            GMT.loadCustomisationPreset("G4SMale")
        else
            GMT.loadCustomisationPreset("G4SFemale")
        end
        Wait(2000)
        ...
        Wait(5000)
        TaskVehicleTempAction(GMT.getPlayerPed(), b, 27, 10.0)
        Wait(2000)
...
RegisterNetEvent(
    ...
    function()
        ...
        local C = GMT.getPlayerPed()
        tGMT.setCustomization(json.decode(x))
        if A.active then
            toggleCashbox()
        end
        if k ~= 0 then
...
RegisterNetEvent(
    ...
    function()
        ...
        while not f do
            ...
            local P = #(GMT.getPlayerCoords() - ae)
            if P <= 1.0 then
                ad = true
        ...
        TriggerServerEvent("GMT:updateMoneyInVehicle")
        t = false
    end
)
RegisterNetEvent(
    "GMT:requestMoneyOutVehicle",
    function()
        local ac = GMT.getPlayerPed()
        while IsPedInAnyVehicle(ac, true) do
            Wait(400)
        ...
        w = 1
        TriggerServerEvent("GMT:updateMoneyOutVehicle")
    end
)
...
RegisterNetEvent(
    "GMT:activeVansUpdate",
    function(ax, O, aC)
        if not a then
            local b = GMT.getObjectId(ax, "GMT:activeVansUpdate 1")
            if b ~= 0 and b and DoesEntityExist(b) then
                if aC then
                    s[ax] = nil
                else
                    if not table.has(s, ax) then
                        local b = GMT.getObjectId(ax, "GMT:activeVansUpdate")
                        s[ax] = {O, b, ax}
                    end
                end
            end
