...
RegisterCommand("taco",function(j, k, l)
    local m = GetEntityCoords(GMT.getPlayerPed())
    local n = GetVehiclePedIsIn(GMT.getPlayerPed())
    local o = GetEntityModel(n)
    if not a then
        if globalOnTacoDuty then
            if o == `taco` then
                GMT.notify("~g~Now selling tacos!")
                SetVehicleDoorOpen(n, 5, true, true)
                tacoVehicle = n
                a = true
                FreezeEntityPosition(n, true)
                TriggerServerEvent("GMT:addTacoSeller", m, 15000)
                while GetVehiclePedIsIn(GMT.getPlayerPed(), false) == tacoVehicle do
                    Wait(100)
                end
                GMT.notify("~r~Stopped selling tacos!")
            ...
            GMT.notify("~r~You do not have the taco seller job!")
        end
    end
end,false)
...
Citizen.CreateThread(function()
    while true do
        ...
        for g, s in pairs(d) do
            ...
            while #(GetEntityCoords(GMT.getPlayerPed()) - s.position) <= 8 do
                Wait(100)
            end
            b = nil
            c = nil
