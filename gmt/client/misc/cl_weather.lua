...
function GMT.setTime(f, g, h)
    b = {h = f, m = g, s = h}
end
function GMT.overrideTime(f, g, h)
    b.h = f
    b.m = g
    b.s = h
    c = true
end
function GMT.cancelOverrideTimeWeather()
    c = false
end
function GMT.setWeather(i)
    a = i
end
...
GMT.createThreadOnTick(func_manageTimeAndWeather)
RegisterNetEvent("GMT:setTime",function(f, g, h)
    GMT.setTime(f, g, h)
end)
RegisterNetEvent("GMT:setWeather",function(i)
    GMT.setWeather(i)
end)
...
function voteWeather(C)
    if j then
        if not k then
            TriggerServerEvent("GMT:vote", C)
            k = true
            GMT.notify("~g~Vote sent!")
        else
            GMT.notify("~r~You have already voted!")
        end
    else
        GMT.notify("~r~Vote not in progress, start a vote with /voteweather!")
    end
end
function resetVotes()
    m = {
...
RegisterNetEvent("GMT:startWeatherVote",function()
    j = true
...
RegisterNetEvent("GMT:voteStateChange",function(D)
    m[D] = m[D] + 1
end)
RegisterCommand("voteweather",function()
    TriggerServerEvent("GMT:tryStartWeatherVote")
end,false)
