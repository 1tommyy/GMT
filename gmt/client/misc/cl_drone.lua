...
local function d()
    ...
    if c.cameraEnabled then
        ...
        local f = 500 * math.ceil(#(GMT.getPlayerCoords() - GetEntityCoords(c.vehicleHandle)) / 10)
        ...
        local g = GMT.loadModel("prop_cs_tablet")
        c.tablet = CreateObject(g, GMT.getPlayerCoords(), true)
        ...
        GMT.loadAnimDict("amb@code_human_in_bus_passenger_idles@female@tablet@idle_a")
        TaskPlayAnim(
            e,
...
RegisterNetEvent(
    ...
    function()
        if not c.active then
            if GMT.getPlayerVehicle() == 0 and not tGMT.isInComa() then
                TriggerServerEvent("GMT:performDroneLog", "Powered on their DJI Inspire Drone")
                newDrone()
            else
                GMT.notify("You are not able to use a drone right now.")
            end
        else
            TriggerServerEvent("GMT:performDroneLog", "Powered off their DJI Inspire Drone")
            deleteDrone()
        end
    end
)
        ...
        while true do
            if c.active then
                ...
                if not NetworkHasControlOfEntity(c.driverHandle) or not NetworkHasControlOfEntity(c.vehicleHandle) then
                    GMT.notify("You have ~b~lost control ~w~of the drone, after going out of range")
                    deleteDrone()
                end
...
Citizen.CreateThread(
    function()
        while true do
            local p = GMT.getPlayerVehicle()
            if p ~= 0 then
                local q = GetEntityModel(p)
                if q == GetHashKey(a) then
                    o = o + 1
                    if o > 3 then
                        GMT.notify("~r~Access Denied~w~: Engaging defences in 3, 2, 1...")
                        Wait(500)
                        local r = GMT.getPlayerCoords()
                        AddOwnedExplosion(GMT.getPlayerPed(), r.x, r.y, r.z, 4, 17.0, true, false, 10.0)
                        ...
                        if GMT.getPlayerVehicle() and DoesEntityExist(GMT.getPlayerVehicle()) then
                            DeleteEntity(GMT.getPlayerVehicle())
                        end
                    end
                end
            end
...
function newDrone()
    local g = GMT.loadModel(a)
    local v = GMT.loadModel("s_m_m_bouncer_01")
    local e = GMT.getPlayerPed()
    local r = GetOffsetFromEntityInWorldCoords(e, 0.0, 4.0, 0.0)
    local w = GetEntityHeading(e)
    c.vehicleHandle = GMT.spawnVehicle(a, r, w, true)
    SetModelAsNoLongerNeeded(g)
    SetEntityNoCollisionEntity(GMT.getPlayerPed(), c.vehicleHandle, 1)
    Wait(1000)
    ...
    c.active = true
    GMT.notify("Drone ~b~created~w~.")
    Wait(1000)
    if globalNHSOnDuty then
        GMT.notify("~y~Reminder~w~: These drones are reserved for usage by HEMS, Specialised Off Road Unit")
        GMT.notify("and Senior Pilots only. If you are not in these divisions, you ~r~should not ~r~be using this.")
    end
    GMT.notify("To remove your drone, use ~b~/drone")
end
...
local function B()
    ...
    if p == 0 then
        if z then
            if #(z - GMT.getPlayerCoords()) > 20.0 then
                local C = PlayerPedId()
    ...
    if #(z - D) > 20.0 then
        ...
        if IsDisabledControlJustPressed(0, 23) or IsDisabledControlJustPressed(0, 75) then
            GMT.notify("You must be near the ~y~start position~w~ to exit.")
        end
    end
end
GMT.createThreadOnTick(B)
