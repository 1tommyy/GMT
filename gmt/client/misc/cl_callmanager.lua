...
RegisterNetEvent("GMT:addEmergencyCall",function(o, p, q, r, s, t)
    ...
    if t == "admin" and GMT.getStaffLevel() > 0 then
        if GMT.isNewPlayer() then
            GMT.notify("~b~Admin ticket received.")
        else
            GMT.notify("~b~Admin ticket received.")
        end
        ...
        j()
        e[o] = r
    elseif t == "met" and GMT.globalOnPoliceDuty() then
        if GMT.isNewPlayer() then
            GMT.notify("~b~MET Police call received.")
        else
            GMT.notify("~b~MET Police call received.")
        end
        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
        ...
        j()
        e[o] = r
    elseif t == "nhs" and GMT.globalNHSOnDuty() then
        if GMT.isNewPlayer() then
            GMT.notify("~b~NHS call received.")
        else
            GMT.notify("~b~NHS call received.")
        end
        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
        ...
        j()
        e[o] = r
    elseif t == "aa" and GMT.globalOnAADuty() then
        if GMT.isNewPlayer() then
            GMT.notify("~b~AA call received.")
        else
            GMT.notify("~b~AA call received.")
        end
        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
...
Citizen.CreateThread(
    function()
        while true do
            if a and GMT.getStaffLevel() > 0 then
                ...
                DrawAdvancedText(0.339, 0.522, 0.005, 0.0028, 0.61, "GMT CALL MANAGER", 255, 255, 255, 255, GMT.getFontId("Akrobat-ExtraBold"), 0)
                local w = 0
                    ...
                    if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                        ...
                        if h then
                            ...
                            if C then
                                ...
                                GMT.notify("~r~No calls available.")
                            end
                        end
                    end
                else
...
Citizen.CreateThread(
    function()
        while true do
            if IsControlJustPressed(0, 243) and (GMT.getStaffLevel() > 0 or GMT.globalOnAADuty() or GMT.globalNHSOnDuty()  or GMT.globalOnPoliceDuty()) then
                a = not a
                if not a then
                    TriggerEvent("GMT:showHUD", true)
                    globalHideEmergencyCallUI = false
