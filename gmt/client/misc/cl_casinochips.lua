...
RMenu.Add('GMTChips','cashier',RageUI.CreateMenu("","",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('GMTChips','cashier'):SetSubtitle("Chips")
RMenu.Add('casino_enter','casino',RageUI.CreateMenu("","",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_enter','casino'):SetSubtitle("Enter")
RMenu.Add('casino_exit','casino',RageUI.CreateMenu("","",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_exit','casino'):SetSubtitle("Exit")
RMenu.Add('casino_rooftop_exit','casino',RageUI.CreateMenu("","",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_rooftop_exit','casino'):SetSubtitle("Exit")
local h={{model='u_f_m_casinocash_01',pedPosition=vector3(1117.671,218.7382,-49.4),pedHeading=100.0,entryPosition=vector3(1116.0776367188,218.04238891602,-50.435089111328)},{model='u_f_m_casinocash_01',pedPosition=vector3(20.152572631836,6705.5737304688,-105.85441589355),pedHeading=75.0,entryPosition=vector3(18.445615768433,6705.7485351563,-106.85442352295), entryPosition=vector3(412.78302001953,9.3809432983398,90.935417175293)}}
Citizen.CreateThread(function()
    for i, j in pairs(h) do
        tGMT.createDynamicPed(j.model,j.pedPosition,100.0,true,"mini@strip_club@idles@bouncer@base","base",75.0,nil,function(k)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('GMTChips', 'cashier')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Buy chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    ...
                    if tonumber(q)then 
                        q=tonumber(q)
                        if q>0 then 
                            TriggerServerEvent("GMT:buyChips",q)
                        else 
                            GMT.notify("~r~Negative amount.")
                        end 
                    else 
            ...
            RageUI.ButtonWithStyle("Buy all chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    TriggerServerEvent("GMT:buyChips")
                end 
            end)
            RageUI.ButtonWithStyle("Sell chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    ...
                    if tonumber(q)then 
                        q=tonumber(q)
                        if q>0 then 
                            TriggerServerEvent("GMT:sellChips",q)
                        else 
                            GMT.notify("~r~Negative amount.")
                        end 
                    else 
            ...
            RageUI.ButtonWithStyle("Sell all chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    TriggerServerEvent("GMT:sellChips")
                end 
            end)
        end)
    end
    if RageUI.Visible(RMenu:Get('casino_exit', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Exit Diamond Casino","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    TriggerServerEvent("GMT:exitDiamondCasino")
                    tGMT.teleport(e.x,e.y,e.z)
                    if cardObjects then 
    ...
    if RageUI.Visible(RMenu:Get('casino_enter', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Enter Diamond Casino","",{RightLabel="→→→"},true,function(n,o,p)
                if p and GMT.getPlayerCombatTimer() == 0 then 
                    TriggerServerEvent("GMT:enterDiamondCasino")
                    tGMT.teleport(f.x,f.y,f.z)
                end 
            end)
            RageUI.ButtonWithStyle("Diamond Casino Rooftop","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    tGMT.teleport(g.x,g.y,g.z)
                end 
            end)
        end)
    end
...
function showCasinoChipsCashier(u)
    ...
    RageUI.Visible(RMenu:Get('GMTChips','cashier'),u)
end
...
Citizen.CreateThread(function()
    while true do 
        local v=GetEntityCoords(GMT.getPlayerPed())
        a=#(v-e)
        b=#(v-f)
        c=#(v-g)
        Wait(100)
...
RegisterNetEvent("GMT:setDisplayChips")
AddEventHandler("GMT:setDisplayChips",function(x)
    d=getMoneyStringFormatted(tostring(x))
end)

RegisterNetEvent("GMT:chipsUpdated",function()
...
AddEventHandler("GMT:onClientSpawn",function(D, E)
    if E then
        ...
        for H,k in pairs(h)do 
            tGMT.addBlip(k.entryPosition.x,k.entryPosition.y,k.entryPosition.z,683,0,"Chips Cashier",0.7,true)
            tGMT.addMarker(k.entryPosition.x,k.entryPosition.y,k.entryPosition.z,1.0,1.0,1.0,138,43,226,70,50,27)
            GMT.createArea("chipscashier_"..H,k.entryPosition,1.5,6,C,E,F,{})
        end
        for I,J in pairs(z)do 
            GMT.createArea("casinos_"..I,J.position,J.radius,6,function()
            end,function()
            end,G,{})
        end 
    end
