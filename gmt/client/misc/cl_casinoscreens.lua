...
function jimmy()
    ...
    while p do 
        if#(GMT.getPlayerCoords()-t)<4.0 then 
...
AddEventHandler("GMT:enteredDiamondCasino",function()
    insideDiamondCasino=true;
    startCasinoThreads()
end)
AddEventHandler("GMT:exitedDiamondCasino",function()
    insideDiamondCasino=false 
end)
AddEventHandler("GMT:bigWinDiamondCasino",function()
    if not insideDiamondCasino then 
        return 
    end;
    w=true 
