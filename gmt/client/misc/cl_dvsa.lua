...
local j =
    tGMT.addBlip(i.test.reception.x, i.test.reception.y, i.test.reception.z, 523, 47, "DVSA Test Centre", 1.0, false)
local k = function()
...
GMT.createArea("dvsaTestCentre_", i.test.reception, 1.5, 6, k, l, m, {})
RegisterNetEvent(
    "GMT:dvsaData",
    function(n, o, p, q)
        h = true
...
RegisterNetEvent(
    "GMT:dvsaMessage",
    function(w, x, y)
        GMT.notifyPicture(i.images.dict, i.images.govLarge, y, w, x)
    end
)
RegisterNetEvent(
    "GMT:beginTestClient",
    function(z, A)
        if z then
            TriggerServerEvent("GMT:dvsaBucket", false)
            currentTest = {
                active = false,
                ped = 0,
                vehicle = 0,
            ...
            local B = GMT.loadModel(i.test.examinerModel)
...
function handleTestRoute()
    ...
    while currentTest.active do
        if not C then
            ...
            local E = #(GMT.getPlayerCoords() - i.test.routes[currentTest.route][currentTest.waypoint + 1].coords)
            ...
            if E < 3.5 then
                ...
                if currentTest.waypoint >= #i.test.routes[currentTest.route] then
                    ...
                    if currentTest.blip ~= 0 then
                        tGMT.removeBlip(currentTest.blip)
                    end
                    if currentTest.marker ~= 0 then
                        tGMT.removeMarker(currentTest.marker)
                    end
                    returnToTestCentre()
                else
...
function policeChase()
    ...
    local I = GMT.spawnVehicle(G, H, 270.93, false, true)
    ...
    GMT.loadModel(M)
    local N = GMT.spawnVehicle(M, H, 270.93, false, true)
...
function returnToTestCentre()
    ...
    local T = false
    while true do
        local E = #(GMT.getPlayerCoords() - i.test.parkingSpaces[currentTest.parkingSpace].coords)
        if E < 15.0 then
...
function finishTest(S)
    ...
    if S then
        ...
        for r, s in pairs(currentTest.seriousReason) do
            ...
            GMT.notify("~r~Serious Fault~w~: " .. currentTest.seriousReason[r])
            Wait(500)
        end
        GMT.notifyPicture(i.images.dict, i.images.govLarge, "Your ~y~minor ~w~faults were:", "UK Government", "DVSA")
        Wait(3000)
        dvsaSound("minorFaults")
        for r, s in pairs(currentTest.minorsReason) do
            U = U .. ", " .. currentTest.minorsReason[r]
            GMT.notify("~y~Minor Fault~w~: " .. currentTest.minorsReason[r])
            Wait(500)
        end
        Wait(4000)
    ...
    if tGMT.teleport ~= nil then
        tGMT.teleport(i.test.finishTestTpCoords.x, i.test.finishTestTpCoords.y, i.test.finishTestTpCoords.z)
    end
    Wait(3000)
    SetTimeout(
        20000,
    ...
    TriggerServerEvent("GMT:dvsaBucket", true)
    if S then
        TriggerServerEvent("GMT:candidatePassed", currentTest.serious, currentTest.minors, U)
        currentTest.active = false
    else
        TriggerServerEvent("GMT:candidateFailed", currentTest.serious, currentTest.minors, V, U)
        currentTest.active = false
    end
    if currentTest.blip ~= 0 then
        tGMT.removeBlip(currentTest.blip)
...
function issueSerious(W)
    ...
    useTablet()
    if GMT.isDev() then
        GMT.notify("~r~Serious Fault~w~: " .. W)
    end
end
...
function pullUpOnRight()
    ...
    GMT.notifyPicture(i.images.dict, i.images.govLarge, "Listen carefully for directions", "Controlled Stop", "DVSA")
    local T = false
    local Y = false
...
function operateHeadlights()
    PlaySoundFrontend(-1, "Out_Of_Bounds_Timer", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 1)
    GMT.notifyPicture(i.images.dict, i.images.govLarge, "Listen carefully for directions", "Show Me Question", "DVSA")
...
function emergencyStop()
    ...
    GMT.notifyPicture(i.images.dict, i.images.govLarge, "Listen carefully for directions", "Test Instruction", "DVSA")
    currentTest.subtitle =
    ...
    GMT.notifyPicture(i.images.dict, i.images.govLarge, "Perform an emergency stop immediately", "STOP", "DVSA")
    Wait(3000)
    if not GetEntitySpeed(currentTest.vehicle) == 0.0 then
...
function useTablet()
    local B = GMT.loadModel("prop_cs_tablet")
                ...
                function()
                    ...
                    if GMT.isDev() or GMT.getUserId() == 2 then
                        RageUI.Button(
                            ...
                            function(a8, a9, aa)
                                if aa then
                                    TriggerServerEvent("GMT:obtainFullLicence", source)
                                end
                            end--,
                            --RMenu:Get("dvsa", "tests")
                        )
                                    ...
                                    function(a8, a9, aa)
                                        if aa then
                                            if a.banned then
                                                ...
                                                if not currentTest.requested then
                                                    TriggerServerEvent("GMT:beginTest")
                                ...
                                RageUI.Button(
                                    ...
                                    function(a8, a9, aa)
                                        if aa then
                                            ...
                                            TriggerServerEvent("GMT:surrenderLicence")
                                        end
                                    end
                                )
                            end
                            ...
                            if a.active then
                                ...
                                if not f then
                                    RageUI.Button(
                                        ...
                                        function(a8, a9, aa)
                                            if aa then
                                                TriggerServerEvent("GMT:activateLicence")
                                                f = true
                                                GMT.notifyPicture(
                                                    i.images.dict,
                                                    i.images.govLarge,
    ...
    function()
        while true do
            ...
            if not a.full and not a.banned and not currentTest.active then
                local vehicle = GMT.getPlayerVehicle()
                if vehicle ~= 0 and GetPedInVehicleSeat(vehicle, -1) == GMT.getPlayerPed() then
...
function flashCamera(ai)
    ...
    TriggerServerEvent(
        "GMT:speedCameraFlashServer",
        math.ceil(GetEntitySpeed(GetVehiclePedIsIn(GMT.getPlayerPed(), false)) * 2.2369)
    )
...
function createPlate(vehicle)
    e.active = true
    e.vehicle = vehicle
    local B = GMT.loadModel("prop_lplate")
    local t = GMT.getPlayerCoords()
    ...
    GMT.notifyPicture(i.images.dict, i.images.lPlate, i.notifications.lPlatesAdded, "DVSA", "Licence Services")
    GMT.notifyPicture(i.images.dict, i.images.lPlate, i.notifications.lPlatesAdded2, "DVSA", "Licence Services")
end
function makeVehicleCrash(aq, J, H, L, ar)
    GMT.loadModel(aq)
    local I = GMT.spawnVehicle(aq, H, ar, true, true)
    while not DoesEntityExist(I) do
        Wait(0)
    end
