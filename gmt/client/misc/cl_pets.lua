...
RMenu.Add(
    "gmtpets",
    "store",
    RageUI.CreateMenu("", "~s~Store", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight(), "menus", "gmt_petsui")
)
...
RegisterNetEvent("GMT:buildPetCFG")
AddEventHandler("GMT:buildPetCFG", function(u, v, w)
   ...
   -- print("id: bear      p: bear") -- only here cause i want prints to make it look like its done :(
    ...
    GMT.createArea("petStore", d.shop.coords, 1.5, 1.5, l, n, o, {})
    local A = tGMT.addBlip(d.shop.coords.x, d.shop.coords.y, d.shop.coords.z, 442, 26, "Pet Store")
    e = true
    local B =
    ...
    function()
        while true do
            if f.viewingPet then
                ...
                if f.viewingId ~= 0 then
                    GMT.loadModel(d.pets[f.viewingId].model)
...
function ridePet()
    ...
    if #(GMT.getPlayerCoords() - GetEntityCoords(a1, true)) > 2.5 then
...
function petCreate(b)
    ...
    if DoesEntityExist(d.pets[b].info.handle) then
        ...
        petFollow()
        RMenu:Get("gmtpets", "main"):SetSubtitle("Pet: ~w~" .. d.pets[h.id].name)
...
function GMT.hasPetSpawned()
    return h.active
end
GMT.createThreadOnTick(func_petHandler)
