...
AddEventHandler("GMT:applyMorphine", function()
    if not a then
        ...
        local d = "pill"
        GMT.loadAnimDict(c)
        TaskPlayAnim(GMT.getPlayerPed(), c, d, 2.0, 2.0, 2500, 48, 1, 9, false, false, false)
        Wait(2500)
        local e = 0
        while e <= 100 do
        ...
        GMT.notify("~r~Fuck, I don't feel too good...")
        local c = "mp_suicide"
        local d = "pill"
        GMT.loadAnimDict(c)
        TaskPlayAnim(GMT.getPlayerPed(), c, d, 2.0, 2.0, 2500, 48, 1, 9, false, false, false)
...
AddEventHandler("GMT:eatTaco", function()
    if not b then
        ...
        GMT.notify("~r~You dropped the taco on the floor trying to stuff it in your mouth!")
    end
end)
