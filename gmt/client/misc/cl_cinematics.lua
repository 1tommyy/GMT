RMenu.Add(
    "mainmenu",
    "cinematic",
    RageUI.CreateMenu("", "", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight(), "menus", "cinematicmenu")
)
RMenu:Get("mainmenu", "cinematic"):SetSubtitle("GMT Cinematic Editor")
RMenu.Add(
    "load_scene",
    "cinematic",
...
Citizen.CreateThread(
    function()
        m = json.decode(GetResourceKvpString("gmt_scenedata") or "{}")
    end
)
RegisterNetEvent(
    "GMT:openCinematicMenu",
    function()
        a(true)
    end
)
function GMT.createCinematicScene(P)
    m[P] = {}
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Scene created & saved.")
end
function tGMT.deleteCinematicScene(P)
    m[P] = nil
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Scene deleted & saved.")
end
function tGMT.createCamera(P, Q)
    if #m[P] == 0 then
        Q.transition = 100
    end
    table.insert(m[P], Q)
    ...
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Camera created & saved.")
end
function tGMT.modifyCamera(P, Q, R)
    m[P][R] = Q
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Camera modified & saved.")
end
function tGMT.deleteCamera(P, S)
    table.remove(m[P], S)
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Camera deleted & saved.")
end
function tGMT.modifyTransition(n, q, T)
    m[n][q].transition = tonumber(T)
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Transition change saved.")
end
function tGMT.modifyCameraBlending(n, q, U)
    m[n][q].blending = l[U]
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Camera Blending change saved.")
end
function tGMT.modifyScreeneffect(n, q, V)
    m[n][q].screeneffect = V
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Screeneffect change saved.")
end
function tGMT.modifyTimecycleEffect(n, q, W)
    m[n][q].timecycleEffect = W
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Timecycle Effect change saved.")
end
function GMT.setTimecycleIntensity(n, q, f)
    m[n][q].timecycleIntensity = f
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Timecycle intensity change saved.")
end
function tGMT.modifyShakeEffect(n, q, X)
    m[n][q].shake = X
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Shake change saved.")
end
function tGMT.modifyCameraType(n, q, type)
    m[n][q].type = type
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Camera type change saved.")
end
function tGMT.addCameraFocus(n, q, Y, Z)
    m[n][q].focusType = Y
    m[n][q].focusData = Z
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~Follow player change saved.")
end
function tGMT.setCameraFov(n, q)
    m[n][q].fov = i[A]
    SetResourceKvp("gmt_scenedata", json.encode(m))
    GMT.notify("~g~FOV change saved.")
end
                ...
                if q and m[n][q] then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(a1, a2, a3)
                            if a3 then
                                ...
                                if tonumber(T) then
                                    tGMT.modifyTransition(n, q, T)
                                else
                                    GMT.notify("~r~Failed to change transition time.")
                                end
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                    ...
                    RageUI.List(
                        ...
                        function(a1, a2, a3, a8)
                            if a8 ~= x then
                                x = a8
                                tGMT.modifyCameraBlending(n, q, k[x])
                            end
                        end,
                        function()
                        end,
                        ...
                        function(a1, a2, a3, a8)
                            ...
                            if a8 ~= e then
                                ...
                                if m[n][q].timecycleEffect ~= "" then
                                    ...
                                    GMT.setTimecycleIntensity(n, q, f[e])
                                end
                            end
                        end,
                        function()
            ...
            function()
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(a1, a2, a3)
                        if a3 then
                            if o then
                                GMT.notify("~r~Can not add a camera whilst you are adding a camera.")
                                return
                            end
                            previewingCamera = nil
                            t = GetEntityCoords(PlayerPedId())
                ...
                for g = 1, #c do
                    RageUI.ButtonWithStyle(
                        ...
                        function(a1, a2, a3)
                            ...
                            if a3 then
                                tGMT.modifyScreeneffect(n, q, c[g])
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                    )
                ...
                for g = 1, #d do
                    RageUI.ButtonWithStyle(
                        ...
                        function(a1, a2, a3)
                            ...
                            if a3 then
                                tGMT.modifyTimecycleEffect(n, q, d[g])
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                    )
                ...
                for g = 1, #h do
                    RageUI.ButtonWithStyle(
                        ...
                        function(a1, a2, a3)
                            ...
                            if a3 then
                                tGMT.modifyShakeEffect(n, q, h[g])
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                    )
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(a1, a2, a3)
                        if a3 then
                            ...
                            if a9 ~= "" then
                                tGMT.addCameraFocus(n, q, "player", a9)
                            end
                        end
                    end,
                    RMenu:Get("camera_manager", "cinematic")
                        ...
                        if a3 then
                            tGMT.clientPrompt(
                                ...
                                function(aa)
                                    ...
                                    tGMT.addCameraFocus(n, q, "coord", vector3(ad, ae, af))
                                end
                            )
                        end
                    end,
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            if (o or p) and noclipActive and GMT.getPlayerVehicle() ~= 0 then
                GMT.notify("~r~You may not noclip when in a vehicle.")
                GMT.toggleNoclip()
            end
                ...
                if a0.focusType ~= "" then
                    if a0.focusType == "player" then
                        ...
                        if aj then
                            ...
                            print("[GMT Cinematic] Failed to point cam at player, could not get entity.")
                        end
...
Citizen.CreateThread(
    function()
        while true do
            local al, am = GMT.getPlayerVehicle()
            ...
            if an ~= 0 and IsEntityAPed(an) and IsPedAPlayer(an) and not IsEntityVisible(an) then
                ...
                if ao ~= -1 then
                    ...
                    if ap > 0 then
                        local aq = GMT.clientGetUserIdFromSource(ap)
                        if not GMT.clientGetPlayerIsStaff(aq) then
