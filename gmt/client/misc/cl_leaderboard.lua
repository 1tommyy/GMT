local a = {
    ...
    rows = {
        ...
        {"Tayser", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"GMT", "~r~RED TEAM", "", false, 5, 0, 2, "0.5"},
        {"GMT", "~b~BLUE TEAM", "", false, 5, 0, 2, "1.4"},
        {"ZK", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"GMT", "~r~RED TEAM", "", false, 5, 2, 0, "0.5"},
        {"GMT", "~b~BLUE TEAM", "", false, 5, 2, 0, "1.4"},
...
RegisterCommand(
    ...
    function()
        if GMT.isDev() then
            local b = tGMT.createLeaderboard(a)
            Wait(1000)
            tGMT.closeLeaderboard(b)
        end
    end,
    false
)
...
function tGMT.closeLeaderboard(b)
    c[b] = false
end
function tGMT.createLeaderboard(m)
    local n = Scaleform("SC_LEADERBOARD")
    n.RunFunction("SET_DISPLAY_TYPE", {1})
    n.RunFunction("SET_MULTIPLAYER_TITLE", {"GMT RP Event"})
...
function tGMT.updateScoreboard(u, v, w)
    local x
    ...
    if u == -1 then
        ...
        if x then
            ...
            print("error: GMT:updateScoreboard couldn't find row with values", u, v, w)
        end
    end
end
RegisterNetEvent(
    "GMT:updateScoreboard",
    function(u, v, w)
        tGMT.updateScoreboard(u, v, w)
    end
)
function tGMT.setPlayerCanOpenLeaderboard(F)
    e = F
end
function tGMT.clearLeaderboardData()
    d = {columns = {}, rows = {}}
end
function tGMT.isLeaderboardSetup()
    return table.count(d.rows) > 0
end
function tGMT.removePlayerFromLeaderboard(u)
