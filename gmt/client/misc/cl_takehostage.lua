    ...
    if not a.InProgress and u then
        ...
        if i then
            ...
            if w ~= -1 then
                ...
                if x then
                    GMT.setWeapon(PlayerPedId(), foundWeapon, true)
                    a.InProgress = true
                    a.startTime = GetGameTimer()
                    a.targetSrc = w
                    TriggerServerEvent("GMT:takeHostageSync", w)
                    a.type = "agressor"
                else
...
RegisterNetEvent("GMT:takeHostageSyncTarget",function(y)
...
RegisterNetEvent("GMT:takeHostageReleaseHostage",function()
    a.InProgress = false
    a.type = ""
    DetachEntity(PlayerPedId(), true, false)
    q("reaction@shove")
...
RegisterNetEvent("GMT:takeHostageKillHostage",function()
    a.InProgress = false
    a.type = ""
...
RegisterNetEvent("GMT:takeHostageCl_stop",function()
    a.InProgress = false
    a.type = ""
    ClearPedSecondaryTask(PlayerPedId())
    DetachEntity(PlayerPedId(), true, false)
    ...
    while true do
        if a.type == "agressor" then
            ...
            if IsEntityDead(PlayerPedId()) then
                ...
                RemoveAnimDict("reaction@shove")
                TriggerServerEvent("GMT:takeHostageReleaseHostage", a.targetSrc)
            end
            ...
            elseif IsDisabledControlJustPressed(0, 74) and GetGameTimer() - a.startTime > 4000 then
                ...
                RemoveAnimDict("anim@gangops@hostage@")
                TriggerServerEvent("GMT:takeHostageKillHostage", a.targetSrc)
                TriggerServerEvent("GMT:takeHostageStop", a.targetSrc)
                Wait(100)
...
function GMT.takeHostageInProgress()
    return a.InProgress
end
