...
RegisterCommand("flagf",function(o, p)
    if c ~= "" then
        ...
        if table.count(p) ~= 0 then
            q = table.concat(p, " ")
            TriggerServerEvent("GMT:flagVehicleAnpr", string.upper(c), q)
        else
            GMT.notify("Error~w~: You must provide a valid flag reason.")
        end
    end
end)
RegisterCommand("flagr",function(o, p)
    if d ~= "" then
        ...
        if table.count(p) ~= 0 then
            q = table.concat(p, " ")
            TriggerServerEvent("GMT:flagVehicleAnpr", string.upper(d), q)
        else
            GMT.notify("Error~w~: You must provide a valid flag reason.")
        end
    end
...
GMT.createThreadOnTick(func_drawFullAnpr)
function anprGetVehicle(u)
    ...
    local w = GetVehiclePedIsIn(GMT.getPlayerPed(), false)
    ...
    if B > 0 and IsEntityAVehicle(B) then
        ...
        if C then
            if m[C] then
                ...
                GMT.notify(string.format("ANPR Alert~w~: Plate ~b~%s ~w~is flagged for:~b~~n~%s", C, D))
                n = true
...
function isPedInPoliceVehicle()
    if IsPedInAnyVehicle(GMT.getPlayerPed(), false) then
        if GetVehicleClass(GMT.getPlayerVehicle()) == 18 and globalOnPoliceDuty then
            return true
        end
    end
    return false
...
RegisterNetEvent("GMT:setFlagVehicles",function(H)
    m = H
end)