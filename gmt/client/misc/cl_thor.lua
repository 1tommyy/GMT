...
RegisterNetEvent("GMT:useTheForceTarget")
AddEventHandler("GMT:useTheForceTarget",function()
    FreezeEntityPosition(PlayerPedId(),true)
    Wait(2500)
    GMT.loadAnimDict("ragdoll@human")
    TaskPlayAnim(GMT.getPlayerPed(),"ragdoll@human","electrocute",3.0,1.0,-1,01,0,0,0,0)
    Wait(5000)
    SetEntityHealth(GMT.getPlayerPed(),0)
    FreezeEntityPosition(PlayerPedId(),false)
end)


RegisterNetEvent("GMT:zapFriendlySync")
AddEventHandler(
    "GMT:zapFriendlySync",
    function(q, r)
...
RegisterNetEvent("GMT:useTheForceSync")
AddEventHandler("GMT:useTheForceSync",function(q,r,s)
    if#(q-GetEntityCoords(PlayerPedId()))<25.0 then 
        TriggerEvent("gmt:PlaySound", "unlimitedpower")
        local s={}
        local a=0
        local diffVector=r-q
        for t=1,10,1 do 
    ...
    if user_id == 1 then
        ...
        if j then  
            ...
            if A ~= -1 then
                ...
                local t = {}
                local s = r - q
                TriggerServerEvent("GMT:theForceSync", A, q, r, s)
                for u = 1, 10, 1 do
                    table.insert(t, q + vector3(s.x / 10 * u, s.y / 10 * u, s.z / 10 * u))
                end
                TriggerEvent("gmt:PlaySound", "unlimitedpower")
                FreezeEntityPosition(PlayerPedId(), true)
                GMT.loadAnimDict("rcmbarry")
                TaskPlayAnim(GMT.getPlayerPed(),"rcmbarry","bar_1_attack_idle_aln",3.0,1.0,-1,01,0,0,0,0)
