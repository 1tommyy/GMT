...
local cd = 0
AddEventHandler("GMT:onClientSpawn",function(b, c)
    if c then
        local d = function(e)
        ...
        local g = function(e)
            ...
            if IsControlJustPressed(1, 38)  then
                if GetGameTimer() - cd > 60000 then
                    local h = GMT.loadModel(a[e.skateparkId].carID)
                    ...
                    SetPedIntoVehicle(GMT.getPlayerPed(), i, -1)
                    SetModelAsNoLongerNeeded(h)
                    cd = GetGameTimer()
                else
                    GMT.notify('Please wait before spawning another BMX.')
                end
            end
        end
        for j, k in pairs(a) do
            GMT.createArea("skatepark_" .. j, k.position, 1.5, 6, d, f, g, {skateparkId = j})
            tGMT.addMarker(k.position.x,k.position.y,k.position.z,1.0,1.0,1.0,255,0,0,170,50,38,false,false,true)
        end
    end
end)
