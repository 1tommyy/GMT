...
RegisterNetEvent("GMT:beginDeliverooJob",function()
    local e = d[math.random(1, #d)]
    b.tempVehicle = GMT.spawnVehicle(e,b.spawnVehicleVector.x,b.spawnVehicleVector.y,b.spawnVehicleVector.z,343,true,true,true)
    b.onJob = true
    GMT.disableCustomizationSave(b.onJob)
    b.previousCustomisation = tGMT.getCustomization()
    local f = PlayerPedId()
    SetPedComponentVariation(f, 5, 126, 0, 0)
    SetPedComponentVariation(f, 6, 0, 0, 0)
    SetPedComponentVariation(f, 11, 449, 0, 0)
    GMT.notify("~g~Deliveroo Job started, exit the restaurant and head to the first drop off.")
    while b.onJob do
...
RegisterNetEvent("GMT:deliverooJobEnd",function(g)
    if g then
        GMT.notify("~g~Shift complete.")
        DeleteVehicle(GetVehiclePedIsIn(GMT.getPlayerPed(), false))
        DeleteVehicle(b.tempVehicle)
    else
        RemoveBlip(b.tempBlip)
        tGMT.removeMarker(b.tempMarker)
    end
    tGMT.setCustomization(b.previousCustomisation)
    b.onJob = false
    GMT.disableCustomizationSave(b.onJob)
    b.tempMarker = 0
    b.tempBlip = 0
...
RegisterNetEvent("GMT:deliverooJobReachedNextStop",function(i)
    ...
    if n then
        ...
        local r = function(p)
            if IsControlJustReleased(1, 38) and not b.onJob then
                TriggerServerEvent("GMT:attemptBeginDeliverooJob")
            end
        end
        tGMT.addBlip(b.startVector.x, b.startVector.y, b.startVector.z, 106, 1, "Deliveroo Job")
        tGMT.addMarker(b.startVector.x,b.startVector.y,b.startVector.z - 1,1.0,1.0,1.0,255,0,0,70,50,38,false,false,true)
        GMT.createArea("deliveroo", b.startVector, 1.5, 6, o, q, r, {})
    end
end)
