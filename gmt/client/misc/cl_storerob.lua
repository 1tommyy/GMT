...
AddEventHandler(
    ...
    function(t, u)
        if u then
            TriggerServerEvent("GMT:getStoreRobBlips")
            while p == nil do
                Wait(0)
            end
            while true do
    ...
    if not IsPedDeadOrDying(x) then
        if IsEntityPlayingAnim(x, "mp_am_hold_up", "holdup_victim_20s", 3) then
            ...
            if not IsPedDeadOrDying(x) then
                ...
                local A = GMT.loadModel("prop_poly_bag_01")
                        ...
                        function()
                            while true do
                                ...
                                if DoesEntityExist(B) then
                                    if #(GetEntityCoords(GMT.getPlayerPed()) - GetEntityCoords(B)) <= 1.5 then
                                        PlaySoundFrontend(
                                            -1,
                                            "ROBBERY_MONEY_TOTAL",
                                        ...
                                        TriggerServerEvent("GMT:pickupCashFromApu", v)
                                        DeleteObject(B)
                                        break
                                    end
                                else
...
Citizen.CreateThread(
    function()
        while true do
            local t = GetEntityCoords(GMT.getPlayerPed(), true)
            if r then
                for v, w in pairs(r) do
...
function refreshStoreBlips(u)
    ...
    for D, E in pairs(p) do
        ...
        if E.beingrobbed == true then
            tempBlip = tGMT.addBlip(F.x, F.y, F.z, 52, 1, "Robbable Store [BEING ROBBED]")
        else
            tempBlip = tGMT.addBlip(F.x, F.y, F.z, 52, 2, "Robbable Store")
        end
...
Citizen.CreateThread(
    function()
        for v, w in pairs(r) do
            local G = GMT.loadModel(w.shopNpcModel)
            local H =
                CreatePed(
                26,
                G,
            ...
            SetPedFleeAttributes(H, 0, 0)
            local I = GMT.loadModel(w.prop_safe)
            local J = GMT.loadModel(w.prop_door)
            local K = GMT.loadModel(w.money_prop)
            local L = GMT.loadModel(w.money_prop2)
            local M = GMT.loadModel(w.money_prop3)
            local N = GMT.loadModel(w.money_prop4)
        ...
        while true do
            local U = GetEntityCoords(GMT.getPlayerPed())
            for v, w in pairs(r) do
                    ...
                    if X ~= 0 then
                        if IsPlayerFreeAimingAtEntity(PlayerId(), X) then
                            if
                             ...
                             then
                                if not q then
                                    if GetSelectedPedWeapon(GMT.getPlayerPed()) == `WEAPON_UNARMED` then
                                        GMT.notify("~r~You need a weapon in your hands to rob this store!")
                                    else
                                        TriggerServerEvent("GMT:initiateStoreRobbery", v)
...
RegisterNetEvent(
    "GMT:beginStoreRobbingAnimations",
    function(v)
...
RegisterNetEvent(
    "GMT:storeRobberyInProgress",
    function(Y, v)
        q = Y
        local Z = GetEntityCoords(GMT.getPlayerPed())
        while q do
            local U = GetEntityCoords(GMT.getPlayerPed())
            if #(Z - U) > 20 then
                TriggerServerEvent("GMT:forceEndRobbery", v)
...
RegisterNetEvent(
    ...
    function(v)
        ...
        local G = GMT.loadModel(r[v].shopNpcModel)
        local H =
            CreatePed(
            26,
            G,
...
function RunMiniGame(v, ab, ac)
    ...
    while safeCrackingInProgress do
        ...
        elseif safeCrackingState == "cracking" then
            ...
            local ae = GetEntityHealth(GMT.getPlayerPed())
            if ae <= 102 or tGMT.isHandcuffed() then
                ...
                if IsControlJustPressed(0, 172) then
                    if af == a[ad] then
                        ...
                        if IsSafeUnlocked(b, ad) then
                            ...
                            safeCrackingInProgress = false
                            ClearPedTasksImmediately(GMT.getPlayerPed())
                        end
                    else
                        ad = 1
                        b = InitSafeLocks(a)
...
function GetSafeDoorAnimOffsetPosition(ah, ai, aj)
    ...
    return vector3(ah.x + ao, ah.y + ap, GetEntityCoords(GMT.getPlayerPed()).z)
end
function PlaySafeCrackIntroAnim(aq, h)
    local ar = "mini@safe_cracking"
    local as = "step_into"
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(GMT.getPlayerPed(), ar, as, aq.x, aq.y, aq.z, 0.0, 0.0, h, 8.0, 8.0, -1, 2, 0.7, 0, 0)
    Wait(0)
    Wait(1000)
    safeCrackingState = "cracking"
end
function HandleSafeDialMovement()
    local at = GMT.getPlayerPed()
    local ar = "mini@safe_cracking"
    local au = ""
    ...
    loadAnimDict(ar)
    if not IsEntityPlayingAnim(GMT.getPlayerPed(), ar, au, 3) then
        TaskPlayAnimAdvanced(GMT.getPlayerPed(), ar, au, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    end
end
function HandleIncorrectMovement(ad, b)
    ...
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(GMT.getPlayerPed(), ar, ax, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    d = "rotation.idle"
end
...
function EndMiniGame(v, ab)
    ...
    Wait(2500)
    TriggerServerEvent("GMT:syncOpenSafeDoor", v)
    safeCrackingState = "setup"
