...
Citizen.CreateThread(function()
	local j = tGMT.addBlip(-552.90362548828, 5326.1733398438, 72.599586486816, 403, 3, "Meth Trader")
	local k = tGMT.addBlip(133.73462677002, -1305.0340576172, 29.268741607666, 403, 4, "Cocaine Trader")
	local l = tGMT.addBlip(103.47916412354, -1937.1091308594, 19.803705215454, 140, 2, "Weed Trader")
	local m = tGMT.addBlip(3569.8334960938, 3662.5893554688, 33.920593261718, 586, 1, "Heroin Trader")
	local n = tGMT.addBlip(-1118.4926757813, 4926.1889648438, 218.35691833496, 150, 1, "Large Arms Trader")
	local o = tGMT.addBlip(5065.6201171875, -4591.3857421875, 2.8652405738831, 150, 1, "Large Arms Trader")
	local p = tGMT.addBlip(1317.0300292969, 4309.8359375, 38.005485534668, 499, 8, "LSD North Trader", 0.8)
	local q = tGMT.addBlip(2485.7429199219, -430.13989257813, 92.992835998535, 499, 8, "LSD South Trader", 0.8)
	local r = {
		{
			title = "Weed Trader",
			type = "radius",
...
AddEventHandler("GMT:onClientSpawn",function(v, w)
	if w then
		TriggerServerEvent("GMT:refreshTurfOwnershipData")
		TriggerServerEvent("GMT:requestDrugPriceUpdate")
	end
end)
RegisterNetEvent("GMT:attackGangCapture",function(x, y)
	f = true
	a = y
	otherMembersStartGangCapture(x, y, h[x].position)
end)
RegisterNetEvent("GMT:updateTurfOwner",function(x, z)
	turfData[x].gangOwner = z
end)
RegisterNetEvent("GMT:defendGangCapture",function(x, y)
	a = y
	e = true
...
RegisterNetEvent("GMT:turfDefenseInterrupt",function()
	e = false
end)
RegisterNetEvent("GMT:initiateGangCaptureCheck",function(y, C)
	b[y] = C
end)
RegisterNetEvent("GMT:gotTurfOwnershipData",function(D)
...
function GMT.isPlayerInTurf()
    for E, F in pairs(h) do
        if #(GMT.getPlayerCoords() - F.position) < F.distance then
            return true
        end
    end
    return false
...
Citizen.CreateThread(function()
	for B, G in pairs(h) do
		tGMT.addMarker(G.position.x,G.position.y,G.position.z,0.7,0.7,0.5,200,50,0,200,50,24,false,false,true)
	end
	for H, I in pairs(i) do
		tGMT.addMarker(I.position.x,I.position.y,I.position.z,0.7,0.7,0.5,0,50,200,200,50,30,false,false,true)
	end
end)
local J = -1
	...
	if w then
		...
		local O = function(M)
			if IsControlJustPressed(1, 51) then
				if not PlayerIsInGang then
					GMT.notify("~r~You need to be in a gang to capture a turf!")
				elseif not f then
					TriggerServerEvent("GMT:refreshTurfOwnershipData")
					TriggerServerEvent("GMT:checkTurfCapture", M.turfId)
				end
				Wait(10000)
			end
		end
		for B, P in pairs(h) do
			GMT.createArea("turf_" .. B, P.position, 1.5, 6, L, N, O, {turfId = B})
		end
		local Q = function(M)
		...
		local S = function(M)
			if M.draw then
				...
				DrawAdvancedText(0.578, 0.349, 0.005, 0.0028, 0.437, "GMT Trader Manager", 255, 255, 255, 255, 4, 0)
						...
						if T and tonumber(T) then
							...
							if T <= 25 and T >= 0 then
								...
								if GetGameTimer() > K + 30000 then
									...
									if B == 1 then
										TriggerServerEvent("GMT:setNewWeedPrice", T)
									elseif B == 2 then
										TriggerServerEvent("GMT:setNewCocainePrice", T)
									elseif B == 3 then
										TriggerServerEvent("GMT:setNewMethPrice", T)
									elseif B == 4 then
										TriggerServerEvent("GMT:setNewHeroinPrice", T)
									elseif B == 5 then
										TriggerServerEvent("GMT:setNewLargeArmsCommission", T)
									elseif B == 6 then
										TriggerServerEvent("GMT:setNewLSDNorthPrice", T)
									elseif B == 7 then
										TriggerServerEvent("GMT:setNewLSDSouthPrice", T)
									end
								else
									GMT.notify("Rate limit, please wait 30s to change the commission.")
								end
							end
						end
					end
		...
		for B, G in pairs(i) do
			GMT.createArea("turfmanager_" .. B, G.position, 1.5, 6, Q, R, S, {turfId = B, draw = false})
		end
	end
end)
RegisterNetEvent("GMT:captureOwnershipReturned",function(B, U, V)
	if not U then
		startGangCapture(B, h[B].position, V)
	else
		GMT.notify("Turf already owned!")
	end
end)
function otherMembersStartGangCapture(x, y, W)
    if b[y] then
        ...
        Citizen.CreateThread(function()
			local tmpMarker = tGMT.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
			while f do
				local X = GetEntityCoords(GMT.getPlayerPed(), false)
				local Y = #(X - W)
				if Y >= 37.5 then
					GMT.notify("Turf capture cancelled, you got too far!")
					f = false
					TriggerServerEvent("GMT:gangDefenseLocationUpdate", y, "Attackers", true)
				elseif GetEntityHealth(GMT.getPlayerPed()) <= 102 then
					GMT.notify("Turf capture failed, you died")
					f = false
					TriggerServerEvent("GMT:gangDefenseLocationUpdate", y, "Attackers", true)
				end
				Wait(1000)
			end
			tGMT.removeMarker(tmpMarker)
		end)
        Citizen.CreateThread(function()
			while f do
				if not d[y] then
					DrawAdvancedText(0.931,0.934,0.005,0.0028,0.49,"TIME REMAINING TILL CAPTURE: " .. timer .. " SECONDS!",23,255,141,255,GMT.getFontId("Akrobat-ExtraBold"),0)
				else
					DrawAdvancedText(0.931,0.954,0.005,0.0028,0.49,"CAPTURE BLOCKED, KILL ENIMES TO CONTINUE TIMER.",255,23,141,255,GMT.getFontId("Akrobat-ExtraBold"),0)
				end
				Wait(0)
			end
		end)
...
function startGangCapture(x, W, y)
    if turfData[x].owned then
        GMT.notify("You already own this turf!")
        TriggerServerEvent("GMT:failCaptureTurfOwned", x)
    else
        TriggerServerEvent("GMT:initiateGangCapture", x, y)
        local timer = g
        while b[y] == nil do
            Wait(0)
        end
        if b[y] then
            ...
            Citizen.CreateThread(function()
				tmpMarker = tGMT.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
				while f do
					local X = GetEntityCoords(GMT.getPlayerPed(), false)
					local Y = #(X - W)
					if Y >= 37.5 then
...
function defendGangCapture(x, a, Z, _, a0)
    ...
    Citizen.CreateThread(function()
		local tmpMarker = tGMT.addMarker(Z, _, a0 - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
		while e do
			local X = GetEntityCoords(GMT.getPlayerPed(), false)
			local Y = #(X - vector3(h[x].x, h[x].y, h[x].z))
			if Y < 37.5 and GetEntityHealth(GMT.getPlayerPed()) > 102 then
				TriggerServerEvent("GMT:gangDefenseLocationUpdate", a, "Defenders", true)
			elseif GetEntityHealth(GMT.getPlayerPed()) > 102 then
				TriggerServerEvent("GMT:gangDefenseLocationUpdate", a, "Defenders", false)
			elseif GetEntityHealth(GMT.getPlayerPed()) <= 102 then
				GMT.notify("Turf defense failed, you died")
				e = false
				TriggerServerEvent("GMT:gangDefenseLocationUpdate", a, "Defenders", false)
			else
				TriggerServerEvent("GMT:gangDefenseLocationUpdate", a, "Defenders", false)
...
RegisterNetEvent("GMT:setBlockedStatus",function(y, a1)
	d[y] = a1
end)
RegisterNetEvent("GMT:gangDefenseSuccess",function()
	e = false
end)
function gangCaptureSuccess(x, a)
    turfData[x].owned = true
    TriggerServerEvent("GMT:gangCaptureSuccess", a)
end
function gangDefenseSuccess()
    TriggerServerEvent("GMT:gangDefenseSuccess", a)
end
