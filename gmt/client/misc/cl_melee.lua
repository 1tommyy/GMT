...
RegisterCommand("togglemelee",function()
    if GMT.getUserId()==1 or GMT.getUserId()==2 or GMT.getUserId()==3 or GMT.isDev() then
        c = not c
        GMT.notify('Melee Combat: '..(c and "~g~Enabled" or "Disabled"))
    end 
end)
function GMT.enablePunching(d)
    c = d
end
...
AddEventHandler("GMT:onClientSpawn",function(h, i)
    if i then
        ...
        for n, o in pairs(a) do
            GMT.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
        end
    end
end)
...
AddEventHandler("GMT:onClientSpawn",function(h, i)
    if i then
        ...
        for n, u in pairs(t) do
            tGMT.addMarker(u.x, u.y, u.z, 1.0, 1.0, 1.0, 0, 0, 255, 100, 50, 27, false, false, true)
            GMT.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
        end
    end
end)
local v = {}
function putGloves()
    ...
    local x = GMT.loadModel("prop_boxing_glove_01")
