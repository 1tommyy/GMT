    ...
    while true do
        if a.active and a.id ~= 0 then
            ...
            if h ~= `WEAPON_UNARMED` and (not b[a.id].allowPistol or not table.has(c, h)) then
                GMT.setWeapon(g, "WEAPON_UNARMED", true)
            end
...
RMenu.Add("policeshields","main",RageUI.CreateMenu("~b~P~w~olice Shields", "Shield Management", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight()))
...
RegisterNetEvent("GMT:toggleShieldMenu",function(l)
...
function removeShield(m)
    if DoesEntityExist(a.prop) then
        ...
        if m then
            GMT.notify("~g~Success~w~: Shield removed")
        end
    end
end
function createShield(n)
    ...
    local o = false
    local g = GMT.getPlayerPed()
    local p = GMT.getPlayerCoords()
    if a.active then
        removeShield(false)
    end
    a.id = n
    local q = GMT.loadModel(b[n].model)
