...
Citizen.CreateThread(function()
    while true do 
        local e=GMT.getPlayerVehicle()
        if e~=0 and GetPedInVehicleSeat(e,-1)==GMT.getPlayerPed()then 
            local f=GetVehicleEngineHealth(e)
            if f<=b and a[e]==nil then 
                local g=GetVehicleClass(e)
                if g~=8 and g~=16 and g~=15 and g~=13 then 
                    TriggerEvent("gmt:PlaySound", "airbag")
                    createAirbags(e)
                end 
            else 
                if a[e]==nil then 
                    d=GetPedArmour(GMT.getPlayerPed())
                end 
            end 
        else 
            d=0 
...
function createAirbags(e)
    ...
    if d>GetPedArmour(GMT.getPlayerPed())then 
        tGMT.setArmour(d)
    end
    d=0
