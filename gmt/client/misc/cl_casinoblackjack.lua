...
RegisterCommand(
    "getcasinotable",
    function()
        local K = GetEntityCoords(GMT.getPlayerPed())
...
Citizen.CreateThread(function()
    ...
    for O = 0, #G.blackjackTables, 1 do
        ...
        tGMT.createDynamicPed(dealerModel,P.dealerPos,P.dealerHeading,true,nil,nil,75.0,nil,function(Q)
            f[O + 1] = Q
            ...
            GMT.loadAnimDict(dealerAnimDict)
...
Citizen.CreateThread(function()
    while true do
        ...
        if h then
                ...
                if tonumber(W) then
                    ...
                    if W > 0 then
                        if not isChairHighRollers(F) and z > B then
                            GMT.notify("~r~Maximum bet is " ..tostring(getMoneyStringFormatted(B)) .." please purchase a membership for higher bets.")
                        else
                            z = W
                        end
                    end
            ...
            if IsControlJustPressed(0, 201) then -- tayser
                if tonumber(z) >= 0 then
                    if not isChairHighRollers(F) and z > B then
                        GMT.notify("~r~Maximum bet is " ..tostring(getMoneyStringFormatted(B)) .." please purchase a membership for higher bets.")
                    elseif isChairHighRollers(F) and z < B then
                        GMT.notify("~r~Minimum bet is " .. tostring(getMoneyStringFormatted(B)))
                    elseif z > 500000000 then
                        GMT.notify("~r~Maximum bet is £500,000,000")
                    else
...
Citizen.CreateThread(function()
    while true do
        if sittingAtBlackjackTable and A then
            SetPedCapsule(GMT.getPlayerPed(), 0.2)
            if IsControlJustPressed(0, 202) and not i then
                ...
                TaskPlayAnim(GMT.getPlayerPed(), blackjackAnimDictToLoad, "sit_exit_left", 1.0, 1.0, 2500, 0)
...
RMenu.Add('gmtblackjack', 'instructions', RageUI.CreateMenu("", "test",1300, 50,"casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get('gmtblackjack', 'instructions'):SetSubtitle("Blackjack Table")
RMenu.Add('gmtblackjack_high', 'instructions', RageUI.CreateMenu("", "test",1300, 50,"casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get('gmtblackjack_high', 'instructions'):SetSubtitle("High Rollers Table")
...
function goToBlackjackSeat(an)
    ...
    c = an
    fVar3 = blackjack_func_217(GMT.getPlayerPed(), blackjack_func_218(c, 0), 1)
    fVar4 = blackjack_func_217(GMT.getPlayerPed(), blackjack_func_218(c, 1), 1)
    fVar5 = blackjack_func_217(GMT.getPlayerPed(), blackjack_func_218(c, 2), 1)
    ...
    TaskGoStraightToCoord(GMT.getPlayerPed(), ao.x, ao.y, ao.z, 1.0, 5000, ap, 0.01)
...
function getNewCardFromMachine(nextCard, Y, ab)
    local aR = GMT.loadModel(nextCard)
...
function requestCard()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(GMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@blackjack@player","request_card",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
...
function putBetOnTable()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(GMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@blackjack@player",getAnimNameFromBet(100),4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(3000,function()
        E = true
    end)
...
function angryIBust()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(GMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_terrible_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function angryILost()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(GMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_bad_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function annoyedIPushed()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(GMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_impartial_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function happyIWon()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(GMT.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_good_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
