...
RMenu.Add('gmthighrollers','casino',RageUI.CreateMenu("","",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('gmthighrollers','casino'):SetSubtitle("M~w~EMBERSHIP")
RMenu.Add('gmthighrollers','confirmadd',RageUI.CreateSubMenu(RMenu:Get('gmthighrollers','casino'),"","~s~A~w~re you sure?",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight()))
RMenu.Add('gmthighrollers','confirmremove',RageUI.CreateSubMenu(RMenu:Get('gmthighrollers','casino'),"","~s~A~w~re you sure?",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight()))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('gmthighrollers', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Remove High Rollers Membership (£0)","~r~This is an irrgmtcable action, you will not receive any money in return.",{RightLabel="→→→"},true,function(b,c,d)
...
Citizen.CreateThread(function()
    ...
    for g,h in pairs(a)do 
        local i=GMT.loadModel('u_f_m_casinocash_01')
...
AddEventHandler("GMT:onClientSpawn",function(D, E)
    if E then
        ...
        for q,h in pairs(a)do 
            ...
            GMT.createArea("casinomembership_"..q,h.entryPosition,1.5,6,m,o,p,{})
        end 
	end
end)