...
function GameEventTriggered(i, j)
    if i == "CEventNetworkEntityDamage" then
        ...
        if
            GMT.getDmgIndcator() and (tGMT.isPlusClub() or tGMT.isPlatClub()) and IsEntityAPed(k) and
                k ~= PlayerPedId() and
                l == PlayerPedId()
         then
            ...
            if x then
                ...
                Citizen.CreateThread(
                    function()
                        while true do
                            if A >= 0 then
                                GMT.DrawText3D(vector3(B.x, B.y, B.z + 0.5), w, 0.5, 4, nil, z[y], true, true)
                                A = A - 10
                            end
                            Wait(0)
                        end
...
RegisterNetEvent("GMT:eatPill")
AddEventHandler("GMT:eatPill", function()
    ...
    if DoesEntityExist(playerPed) and not IsEntityDead(playerPed) then
        RequestAnimDict(animDict)
            ...
            if GMT.getPlayerCombatTimer() > 0 then
                GMT.notify("~r~You can't swallow this pill while in combat.")
            elseif not GMT.isInPurge() then
                GMT.notify("~r~You can only do this during the purge")
            else
            ...
            if health > 0 and health <= 200 then
                ...
                GMT.notify("~r~You took the easy way out.")
            end
        end
    end
end)