...
function tGMT.isCompassEnabled()
    return showCompass
end
function tGMT.setCompassEnabled(f)
    showCompass = f
    SetResourceKvp("gmt_compass", tostring(f))
end
...
Citizen.CreateThread(
    function()
        local g = GetResourceKvpString("gmt_compass") or "false"
        if g == "false" then
            showCompass = false
        else
            showCompass = true
...
local function h(i, j, k, l)
    GMT.DrawText(j, k, i, l.size, 0, 0, {l.colour.r, l.colour.g, l.colour.b, l.colour.a}, true)
end
...
local function m()
    if showCompass and not globalHideUi then
        ...
        if a.compass.followGameplayCam then
            ...
            o = 360.0 - GetEntityHeading(GMT.getPlayerPed())
        end
