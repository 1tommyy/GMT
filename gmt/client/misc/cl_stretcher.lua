...
local function q()
    if IsControlJustReleased(0, 299) then
        if NetworkHasControlOfEntity(c) then
            TriggerServerEvent("GMT:toggleAmbulanceDoors", GMT.getNetId(c, "Stretchers"))
        else
...
local function C(D, E)
    ...
    if not y(D) or b then
        ...
        GMT.loadAnimDict(a.positions[e].animDict)
        DetachEntity(x, false, false)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if d and d ~= 0 then
            ...
            if not b and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and GMT.getPlayerVehicle() == 0 then
            ...
            if IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and c == 0 then
                ...
                if H ~= -1 and globalNHSOnDuty then
                    ...
                    if IsControlJustPressed(0, 183) then
                        ...
                        if L > 0 and M ~= 0 then
                            TriggerServerEvent("GMT:forcePlayerOnToStretcher", L, M)
                        end
                    end
                end
                r(K)
            ...
            if IsControlJustReleased(0, 51) and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and not b then
                if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                    local O = GMT.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "Stretcher Thread")
...
function pushStretcher(x, d)
    Citizen.CreateThread(function()
        ...
        TriggerServerEvent("GMT:stretcherAttachPlayer", P)
        ...
        GMT.loadAnimDict("anim@heists@box_carry@")
        local Q = 0
        while IsEntityAttachedToEntity(d, x) do
            ...
            if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                local O = GMT.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "pushStretcher(ped, nearbyStretcher)")
                if DecorGetBool(O, "has_stretcher_inside") ~= false then
                    if not NetworkHasControlOfEntity(O) then
                        if GetGameTimer() - Q > 200 then
                            TriggerServerEvent("GMT:updateHasStretcherInsideDecor",GMT.getNetId(O, "Stretchers"),false)
                            Q = GetGameTimer()
                        end
                    else
            ...
            if c and c ~= 0 then
                ...
                if IsControlJustReleased(0, 300) and (GetVehicleDoorAngleRatio(c, 4) ~= 0 or IsThisModelAHeli(GetEntityModel(c))) and not R then
                    ...
                    if not NetworkHasControlOfEntity(c) then
                        TriggerServerEvent("GMT:updateHasStretcherInsideDecor", GMT.getNetId(c, "Stretcher"), true)
                    else
                        DecorSetBool(c, "has_stretcher_inside", true)
                    end
                    if not NetworkHasControlOfEntity(d) then
                        TriggerServerEvent("GMT:updateStretcherLocation",GMT.getNetId(d, "Stretchers"),GMT.getNetId(c, "Stretchers"))
                    else
                        DecorSetInt(d, "inside_ambulance_netid", GMT.getNetId(c, "Stretchers"))
                    end
...
RegisterNetEvent("GMT:stretcherAttachPlayer",function(P, S)
...
RegisterCommand("stretcher",function()
    if globalNHSOnDuty then
        ...
        local g = GMT.loadModel("trump-stretcher")
        local D = GMT.spawnVehicle(g, v.x, v.y, v.z, GetEntityHeading(x), false, true, true)
...
RegisterCommand("removestretcher",function()
    if globalNHSOnDuty then
        ...
        if V then
            TriggerServerEvent("GMT:removeStretcher", GMT.getNetId(V, "Stretchers"))
        end
    end
end,false)
RegisterNetEvent("GMT:setStretcherInside",function(P, W)
    local D = GMT.getObjectId(P, "GMT:setStretcherInside")
    DecorSetInt(D, "inside_ambulance_netid", W)
end)
RegisterNetEvent("GMT:setHasStretcherInsideDecor",function(X, Y)
    local c = GMT.getObjectId(X, "GMT:setHasStretcherInsideDecor")
    DecorSetBool(c, "has_stretcher_inside", Y)
end)
RegisterNetEvent("GMT:toggleAmbulanceDoorStatus",function(X)
    local c = GMT.getObjectId(X, "GMT:toggleAmbulanceDoorStatus")
...
function GMT.isUsingStretcher()
    return b
end
RegisterNetEvent("GMT:forcePlayerOnToStretcher",function(P)
    if not NetworkDoesNetworkIdExist(P) then
        return
    end
    local D = NetworkGetEntityFromNetworkId(P)
