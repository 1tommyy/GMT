...
local function i()
    ...
    if j == 0 then
        a = true
        SetResourceKvpInt("gmt_quests_completedall", 1)
        TriggerServerEvent("GMT:claimQuestReward")
    end
    return a
end
local function l()
    hasCompletedAllQuestsInt = GetResourceKvpInt("gmt_quests_completedall")
    if not hasCompletedAllQuestsInt then
        SetResourceKvpInt("gmt_quests_completedall", 0)
    end
    if hasCompletedAllQuestsInt == 0 then
        for m = 1, 50 do
            local n = GetResourceKvpInt("gmt_quests_q" .. m .. "_status")
            if not n then
                SetResourceKvpInt("gmt_quests_q" .. m .. "_status", 0)
                n = 0
            end
            if n == 0 then
                c[m] = true
...
local function o(p)
    c[p] = nil
    SetResourceKvpInt("gmt_quests_q" .. p .. "_status", 1)
    TriggerServerEvent("GMT:setQuestCompleted")
    i()
end
local function q()
    local j = 50
...
Citizen.CreateThread(function()
    ...
    for w, k in pairs(c) do
        ...
        GMT.createArea("quests_prop_id_" .. w,undiscoveredQ.pos,100.0,6,x,z,A,{undiscoveredQID = w, undiscoveredQ = undiscoveredQ})
        local B = function()
        end
        local C = function()
        end
        local A = function(y)
            ...
            if IsControlJustReleased(0, 38) then
                ...
                if q() ~= 50 then
                    ...
                    DoScreenFadeIn(2000)
                    tGMT.removeArea("quests_prop_id_" .. y.undiscoveredQID)
                    tGMT.removeArea("quests_text_id_" .. y.undiscoveredQID)
                else
                    r("~g~+14 Days GMT Club Platinum", "You collected them all!", 600)
                    PlaySoundFrontend(-1, "Hack_Success", "DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS", 1)
                    ShakeGameplayCam("FAMILY5_DRUG_TRIP_SHAKE", 1.0)
        ...
        GMT.createArea("quests_text_id_" .. w,undiscoveredQ.pos,2.0,6,B,C,A,{undiscoveredQ = undiscoveredQ, undiscoveredQID = w})
    end
end)
Citizen.CreateThread(function()
    Wait(5000)
