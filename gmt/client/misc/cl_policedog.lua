RMenu.Add("policedog","main",RageUI.CreateMenu("Dog Support Unit", "Dog Management", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight()))
AddEventHandler("GMT:setPoliceOnDuty",function(a)
    if a then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
AddEventHandler("GMT:setPrisonGuardOnDuty",function(b)
    if b then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
...
local i = "K9"
local j = "K9TARGET"
RegisterNetEvent("GMT:policeDogMenu",function()
    if globalOnPoliceDuty and GMT.hasPoliceCallsign() then
        RMenu:Get("gmtscenemenu", "main"):SetSubtitle("MPD~w~: " .. tGMT.getPoliceCallsign() .. " - " .. tGMT.getPoliceRank() .. " - " .. GMT.getPoliceName())
    end
    if globalOnPrisonDuty and GMT.hasHmpCallsign() then
        RMenu:Get("gmtscenemenu", "main"):SetSubtitle("HMP~w~: " .. GMT.getHmpCallsign() .. " - " .. GMT.getHmpRank() .. " - " .. GMT.getHmpName())
    end
    RageUI.Visible(RMenu:Get("policedog", "main"), true)
end)
RegisterNetEvent("GMT:sendClientRagdoll",function()
    SetPedToRagdoll(GMT.getPlayerPed(), 12000, 12000, 0, 0, 0, 0)
    showNotification("~y~~h~Alert~h~~s~: " .. "You have been bitten by a police dog.")
end)
RegisterNetEvent("GMT:policeDogAttack",function()
...
local m = {breed = 1}
local function n()
    local o = GetResourceKvpString("gmt_policedog_customisations")
...
local function r(s)
    ...
    local q = k[m.breed]
    p[q] = s
    SetResourceKvp("gmt_policedog_customisations", json.encode(p))
end
...
function tpDog()
    local z = GetEntityCoords(GMT.getPlayerPed(), true)
                                    ...
                                    if M < 2.0 then
                                        ...
                                        for O, P in pairs(N) do
                                            if I == GetPlayerPed(P) then
                                                ...
                                                TriggerServerEvent("GMT:performDogLog","Instructed dog to attack ID: " .. Q)
                                                TriggerServerEvent("GMT:serverDogAttack", Q)
                                                H = true
                                                Wait(1000)
    ...
    while f.currentAction == d.SearchNearby and DoesEntityExist(f.handle) do
        ...
        for W, X in pairs(GetActivePlayers()) do
            ...
            if GetVehiclePedIsUsing(Y) == 0 and IsEntityVisible(Y) and #(GetEntityCoords(Y, true) - L) < 5.0 then
                ...
                local _ = GMT.clientGetUserIdFromSource(Z)
                if GMT.getJobType(_) == "" then
        ...
        if #V > 0 then
            ...
            TriggerServerEvent("GMT:policeDogSniffPlayer", a0.source)
        end
        Citizen.Wait(5000)
    end
end
function putInVehicle()
    local a2 = GMT.getNearestVehicle(7.0)
    Citizen.Trace(a2)
...
function removeFromVehicle()
    if IsPedInAnyVehicle(GMT.getPlayerPed(), true) then
...
function newDog(q, a4)
    local a5 = GMT.getPlayerPed()
    if not IsPedInAnyPoliceVehicle(a5) then
        local a6 = GMT.loadModel(q)
        ...
        if DoesEntityExist(f.handle) then
            ...
            TriggerServerEvent("GMT:performDogLog", "Created a new police dog")
        end
    end
end
function deleteDog()
    ...
    f.active = false
    f.handle = 0
    TriggerServerEvent("GMT:performDogLog", "Deleted dog")
end
function dogFollow()
    if not f.active then
        return
    ...
    if f.currentAction ~= d.Follow then
        ClearPedTasks(f.handle)
        TaskFollowToOffsetOfEntity(f.handle, GMT.getPlayerPed(), 0.0, 0.0, 0.0, 7.0, -1, 10.0, true)
        f.currentAction = d.Follow
    else
        ClearPedTasks(f.handle)
        f.currentAction = d.Follow
...
RegisterNetEvent("GMT:policeDogIndicate",function(ad)
