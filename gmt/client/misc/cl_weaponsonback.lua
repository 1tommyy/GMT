...
local function g()
    ...
    local i = tGMT.getWeapons()
    local j = false
    local k = globalOnPoliceDuty and GMT.getPlayerVehicle() ~= 0
    for l in pairs(i) do
        local m = GetHashKey(l)
        local n = a.weapons[m]
        if n then
            local o = (not k or n.type ~= "Heavy") and (GMT.isEmergencyService() or not isInGreenzone) and not GMT.isPlayerInAnimalForm()
