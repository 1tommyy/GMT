...
function GMT.setRedzoneTimerDisabled(e)
    a = e
end
function GMT.isPlayerInRedZone()
    return globalInRedzone
end
local f = 0
local timers = {}
function tGMT.setPlayerCombatTimer(g, h)
    ...
    timers[g] = h
    if GetGameTimer() - f > 2500 or tGMT.isStaffedOn() then
        TriggerServerEvent("GMT:setCombatTimer", g)
        f = GetGameTimer()
    end
end
function tGMT.getPlayerCombatTimer()
    return b, c
end
function GMT.getPlayerCombatTimer()
    return b, c
end
...
Citizen.CreateThread(
    function()
        while true do
            if not a then
                local r = GetEntityCoords(GMT.getPlayerPed())
                globalInRedzone = false
                for s, k in pairs(d) do
                    if i(r, k, false) then
                        globalInRedzone = true
                        local r = GetEntityCoords(GMT.getPlayerPed())
                        if not noclipActive or GMT.getStaffLevel() == 0 then
                            tGMT.setPlayerCombatTimer(30, false)
                        end
                        local t
                        local u = false
                        while not u do
                            r = GetEntityCoords(GMT.getPlayerPed())
                            while i(r, k, true) do
                                r = GetEntityCoords(GMT.getPlayerPed())
                                t = r
                                if
                                    IsPedShooting(GMT.getPlayerPed()) and
                                        GetSelectedPedWeapon(GMT.getPlayerPed()) ~= `WEAPON_UNARMED`
                                 then
                                    tGMT.setPlayerCombatTimer(60, true)
                                end
                                if b == 0 then -- b is the combat timer tayser
                                    DrawAdvancedText(0.955,0.965,0.005,0.0028,
                                    0.467,
                                        ...
                                        70,
                                        255,
                                        GMT.getFontId("Akrobat-ExtraBold"),
                                        0
                                    )
                                end
                                Wait(0)
                            ...
                            if b == 0 then
                                ...
                                local v = k.pos - GetEntityCoords(GMT.getPlayerPed())
                                t = t + v * 0.01
                                if GetVehiclePedIsIn(GMT.getPlayerPed(), false) == 0 then
                                    ...
                                    ReleaseSoundId(w)
                                    tGMT.announceMpBigMsg("~r~WARNING", "Get back in the redzone!", 2000)
                                else
                                    SetEntityCoords(GMT.getPlayerPed(), t.x, t.y, t.z)
                                end
                                SetTimeout(
                                    1000,
                                    function()
...
Citizen.CreateThread(
    function()
        while true do
            if b > 0 then
                if a then
                    tGMT.setPlayerCombatTimer(0, false)
                else
                    b = b - 1
                    if b == 0 then
                        c = false
...
AddEventHandler("GMT:startCombatTimer",function(h)
    if not GMT.isEmergencyService() then
        tGMT.setPlayerCombatTimer(60, h)
    end
end)
local function y()
    if not GMT.isEmergencyService() and not tGMT.isInComa() then
        local z = PlayerPedId()
        ...
        if IsPedShooting(z) and not x[A] then
            tGMT.setPlayerCombatTimer(60, true)
        elseif GetPlayerTargetEntity(GMT.getPlayerId()) and IsControlPressed(0, 24) then
            tGMT.setPlayerCombatTimer(60, true)
        end
    end
    if b > 0 then
        DrawAdvancedText(0.985,0.965,0.005,0.0028,
0.467,
