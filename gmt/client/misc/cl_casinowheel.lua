        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    "Spin the wheel of GMT (50,000 Chips)",
                    "",
                    {RightLabel = "→→→"},
                    true,
                    function(e, f, g)
...
Citizen.CreateThread(
    function()
        ...
        local i = GMT.loadModel("vw_prop_vw_luckywheel_02a")
        local j = GMT.loadModel(c)
...
function goToWheel()
    if not d then
        ...
        local D = false
        while not D do
            local E = GetEntityCoords(GMT.getPlayerPed())
        ...
        SetEntityHeading(GMT.getPlayerPed(), 10.9754)
        Wait(2000)
    ...
    function(D, E)
        if E then
            ...
            for K, L in pairs(a) do
                tGMT.addBlip(L.entryPosition.x, L.entryPosition.y, L.entryPosition.z, 681, 0, "GMT Wheel", 0.7, true)
                GMT.createArea("gmtwheel_" .. K, L.entryPosition, 1.5, 6, F, H, I, {})
            end
        end
    end
)
...
RegisterNetEvent("GMT:spinLuckyWheelReaction")
AddEventHandler(
    "GMT:spinLuckyWheelReaction",
    function(N)
        local y = GMT.getPlayerPed()
