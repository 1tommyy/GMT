...
local j = false
function tGMT.isPlayerHidingInBoot()
    return j
end
local function k(vehicle, l)
    if tGMT.isHandcuffed() or tGMT.isTazed() then
        return false
    end
    ...
    if l and #(GMT.getPlayerCoords() - l) > 1.0 then
        return false
    end
    return true
end
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            local r = GMT.getClosestVehicle(7.0)
            local s = GetVehicleClass(r)
            local t = GetVehicleDoorLockStatus(r)
            if q == 0 and a[s] and GetEntityHealth(GMT.getPlayerPed()) > 102 and not noclipActive then
                if r and r ~= 0 then
                    o = 0
                    local u = GetEntityBoneIndexByName(r, "boot")
                    local v = GetWorldPositionOfEntityBone(r, u)
                    local w = #(v - GMT.getPlayerCoords())
                    if w < 2 then
                            ...
                            if not b(vehicle) then
                                ...
                                if k(vehicle, l) then
                                    if t <= 1 then
                                        ...
                                        Wait(100)
                                        GMT.loadAnimDict("timetable@floyd@cryingonbed@base")
                                        ...
                                        while z and j do
                                            ...
                                            if IsDisabledControlPressed(0, 177) then
                                                if GetVehicleDoorLockStatus(r) <= 1 then
                                                    ...
                                                    GMT.notify("~r~Vehicle is locked, cannot get in boot.")
                                                end
                                            end
                                        ...
                                        GMT.notify("~r~Vehicle is locked, cannot get out of boot.")
                                    end
                                else
                                    GMT.notify("~r~You and the vehicle must remain stationary to get in.")
                                end
                            else
                                GMT.notify("~r~Someone is already in this boot.")
                            end
                        end
                    else
                        ClearHelp(true)
...
RegisterNetEvent("GMT:removeHiddenInBoot",function()
    j = false
end)