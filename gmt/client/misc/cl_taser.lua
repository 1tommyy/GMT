...
RegisterNetEvent("GMT:receiveActivation",function()
    if not tGMT.isStaffedOn() then
        ...
        SetPedToRagdoll(GMT.getPlayerPed(), 5000, 5000, 0, 0, 0, 0)
    end
end)
RegisterNetEvent("GMT:arcSuccessful",function()
    ...
    TriggerServerEvent("GMT:playTaserSound", GetEntityCoords(GMT.getPlayerPed()), "arcsound")
    b = false
...
RegisterNetEvent("playTaserSoundClient",function(f, g)
    local h = GetEntityCoords(GMT.getPlayerPed(), true)
    local i = #(h - f)
    if i <= 15 then
...
RegisterNetEvent("GMT:reloadTaser",function(j)
    if GetSelectedPedWeapon(GMT.getPlayerPed()) == GetHashKey(a.hash) then
        MakePedReload(GMT.getPlayerPed())
...
GMT.createThreadOnTick(func_taserHandler)
...
Citizen.CreateThread(function()
    SetPedMinGroundTimeForStungun(GMT.getPlayerPed(), 15000)
    while true do
        if IsPedBeingStunned(GMT.getPlayerPed()) then
            stunGun()
        end
        Wait(100)
    end
...
function GMT.setStunExitsEarly()
    p = true
end
