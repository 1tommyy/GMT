...
local e = false
local f = false
function GMT.isKnockedOut()
    return a
end
function GMT.putInNearestVehicleAsPassenger(j)
    local k = GMT.getClosestVehicle(j)
    if IsEntityAVehicle(k) then
        for l = 1, math.max(GetVehicleMaxNumberOfPassengers(k), 3) do
            if IsVehicleSeatFree(k, l) then
                SetPedIntoVehicle(GMT.getPlayerPed(), k, l)
                return true
            end
        end
    end
...
function GMT.putInNetVehicleAsPassenger(m)
    local k = GMT.getObjectId(m)
    if IsEntityAVehicle(k) then
...
function GMT.putInVehiclePositionAsPassenger(n, o, p)
    local k = GMT.getVehicleAtPosition(n, o, p)
    if IsEntityAVehicle(k) then
        for l = 1, GetVehicleMaxNumberOfPassengers(k) do
            if IsVehicleSeatFree(k, l) then
                SetPedIntoVehicle(GMT.getPlayerPed(), k, l)
...
RegisterNetEvent("GMT:knockOut",function()
    if not a and not GMT.isInPurge() then
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(0, 323) or IsControlPressed(0, 27) and not IsUsingKeyboard(2) then
            if not globalSurrenderring and not drawInventoryUI and not inGUIGMT and not tGMT.isInComa() and not tGMT.isHandcuffed() and (GMT.canAnim() or tGMT.isTazedByRevive()) then
                DisablePlayerFiring(GMT.getPlayerPed(), true)
                DisableControlAction(0, 22, true)
                DisableControlAction(0, 25, true)
                DisableControlAction(0, 154, true)
                if not IsEntityDead(GMT.getPlayerPed()) then
                    if not c and not GetIsTaskActive(GMT.getPlayerPed(), 298) then
                        c = true
                        GMT.loadAnimDict("missminuteman_1ig_2")    
                        TaskPlayAnim(GMT.getPlayerPed(),"missminuteman_1ig_2","handsup_enter",7.0,1.0,-1,50,0,false,false,false)
                        RemoveAnimDict("missminuteman_1ig_2")
                    end
                end
            end
        end
        if (IsControlJustReleased(1, 323) or IsControlJustReleased(1, 27)) and not globalSurrenderring and c and not tGMT.isInComa() and not tGMT.isHandcuffed() and GMT.canAnim() then
            c = false
        ...
        if a then
            ...
            elseif not tGMT.isInComa() then
                ...
                GMT.notify("~r~You have been knocked out!")
            end
        end
        Wait(0)
    end
...
RMenu.Add("policehandbook","main",RageUI.CreateMenu("~b~P~w~olice Handbook", "Officer Handbook", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight()))
...
RegisterNetEvent("GMT:breathTestResult",function(M,N)
    ...
    TaskPlayAnim(GMT.getPlayerPed(),"weapons@first_person@aim_rng@generic@projectile@shared@core","idlerng_med",1.0,-1,10000,50,0,false,false,false)
...
RegisterNetEvent("GMT:flashWarrantCard",function()
    local E = PlayerPedId()
    local V = GMT.loadModel("prop_fib_badge")
...
RegisterNetEvent("GMT:startSearchingSuspect",function()
    tGMT.setCanAnim(false)
    GMT.loadAnimDict("custom@police")
...
RegisterNetEvent("GMT:startBeingSearching",function(a5)
    ...
    a4 = true
    tGMT.setCanAnim(false)
    GMT.loadAnimDict("custom@suspect")
    local s = GMT.getPlayerPed()
...
RegisterNetEvent("GMT:receivePoliceCallsign",function(ag, ah, ai)
    ...
    ab = true
    if a9 == nil then
        GMT.notify("~r~You are required to have a callsign!")
        Wait(1500)
        GMT.notify("~r~Request a callsign in the channel: ~y~#request-callsign")
    else
...
RegisterNetEvent("GMT:receiveUKBFCallsign",function(ag, ah, ai)
    ac = ah
    ad = ag
    ae = ai
    af2 = true
...
RegisterNetEvent("GMT:receiveLFBCallsign",function(ag, ah, ai)
    ac = ah
    ad = ag
    ab = ai
    ay = true
...
function GMT.getPoliceName()
    return aa
end
function GMT.hasPoliceCallsign()
    return ab
end
function GMT.getLFBCallsign()
    return ay
end
function GMT.getLFBRank()
    return ac
end
function GMT.getLFBName()
    return ab
end
function GMT.hasLFBCallsign()
    return ay
end
function GMT.getUKBFName()
    return af3
end
function GMT.hasUKBFCallsign()
    return af2
end
function GMT.getHmpCallsign()
    return ad
end
function GMT.getHmpRank()
    return ac
end
function GMT.getHmpName()
    return ae
end
function GMT.hasHmpCallsign()
    return af
end
...
GMT.createThreadOnTick(func_drawCallsign)
local aj = 0
local function ak()
    local s = GMT.getPlayerPed()
...
RMenu.Add("trainingWorlds","mainmenu",RageUI.CreateMenu("Training Worlds", "Main Menu", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight()))
local ap = {}
local aq = false
    ...
    if RageUI.Visible(RMenu:Get('trainingWorlds', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            for x, at in pairs(ap) do
                ...
                local av = at.bucket == GMT.getPlayerBucket()
