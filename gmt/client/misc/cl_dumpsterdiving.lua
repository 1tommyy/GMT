                ...
                if dumpster ~= 0 then
                    ...
                    if #(pos - dumpPos) < 1.8 then
                        ...
                        if IsPedInAnyVehicle(ped, false) then
                            GMT.notify("~r~You cannot search dumpsters while in a vehicle.")
                            break
                        else
                            if searched[dumpster] then
                                GMT.notify("~r~Dumpster has recently been searched")
                            else
                                drawNativeNotification("Press ~INPUT_PICKUP~ to search the dumpster.")
                                    if IsControlJustReleased(0, 38) then
                                        TriggerServerEvent('GMT:startDumpsterTimer', dumpster)
                                        searched[dumpster] = true
                                        startSearching(searchTime, 'amb@prop_human_bum_bin@base', 'base', 'GMT:giveDumpsterReward')
                                    end
                                end
                            end
                        end
...
RegisterNetEvent('GMT:removeDumpster')
AddEventHandler('GMT:removeDumpster', function(object)
    searched[object] = nil
end)
...
function startSearching(time, animDict, animation, cb)
    ...
    tGMT.startCircularProgressBar("", 5000, nil)
