...
local function b()
    ...
    TriggerServerEvent("GMT:startThrowSmokeGrenade", c)
    Citizen.Wait(100)
    ...
    TriggerServerEvent("GMT:endThrowSmokeGrenade", c, g)
end
local function h(i)
    local j = GetSelectedPedWeapon(i.playerPed)
    if j == `WEAPON_SMOKEGRENADEGMT` or j == `WEAPON_SMOKEGRENADEGMTPD` then
...
RegisterNetEvent("GMT:displaySmokeGrenade",function(c, g)
    local l = {coords = g, isDeleted = false}
    GMT.createArea("smoke_" .. tostring(c),g,250.0,1000.0,k,m,function()end,l)
    Citizen.Wait(60000)
    tGMT.removeArea("smoke_" .. tostring(c))
    if l.handle then
        StopParticleFxLooped(l.handle, false)
        l.handle = nil
        l.isDeleted = true
