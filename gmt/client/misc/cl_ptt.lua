local a = false
function func_ptt()
    if GMT.getRadioAnim() and GMT.canAnim() and not noclipActive and GMT.isInRadioChannel() then
        local b = GMT.getPlayerPed()
        local c = GMT.getPlayerId()
        if a then
...
GMT.createThreadOnTick(func_ptt)
