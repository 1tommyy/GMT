RMenu.Add(
    "nhs",
    "main",
    RageUI.CreateMenu("NHS", "Status: ~b~Available", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight())
)
RMenu.Add(
    "nhs",
    "equipment",
                    ...
                    if e.active then
                        ...
                        if e.readyToRevive then
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        TriggerServerEvent("GMT:finishRevive", e.perm_id)
                                        e.readyToRevive = false
                                        TriggerServerEvent("GMT:playNhsSound", "shockAdvisedCharging")
                                    end
                                end
                            )
                        else
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        ...
                                        TriggerServerEvent("GMT:playNhsSound", "threeBeeps")
                                    end
                                end
                            )
                        end
            ...
            RageUI.DrawContent(
                ...
                function()
                            ...
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        TriggerServerEvent("GMT:finishRevive", e.perm_id)
                                        e.readyToRevive = false
                                        TriggerServerEvent("GMT:playNhsSound", "carryOutShock")
                                    end
                                end
                            )
                        end
                    ...
                    RageUI.Button(
                        ...
                        function(g, h, i)
                            if i then
                                TriggerServerEvent("GMT:playNhsSound", "metronome")
                            end
                        end
                    )
...
function carryLifePak()
    ...
    f.active = true
    local l = GMT.loadModel("prop_lifepak")
    ...
    Citizen.CreateThread(
        function()
            ...
            if not e.dead then
                ...
                if n == 3 then
                    Wait(3000)
                    TriggerServerEvent("GMT:playNhsSound", "checkPatientAlarm")
                end
...
function beginObs(a, u, v, w, d)
    TriggerServerEvent("GMT:playNhsSound", "threeBeeps")
    Wait(3000)
    TriggerServerEvent("GMT:playNhsSound", "analysingHeartRythm")
    e = {}
    ...
    if e.coma then
        ...
        if IsEntityDead(GetPlayerPed(u)) then
            ...
            if e.readyToRevive then
                ...
                TriggerServerEvent("GMT:playNhsSound", "pushToShock")
            end
        end
    end
end
function connectObsToPatient()
    TriggerServerEvent("GMT:attachLifepakServer")
end
RegisterNetEvent(
    "GMT:clientPlayNhsSound",
    function(x, y)
...
RegisterNetEvent("GMT:returnRevive",function()
    TriggerServerEvent("GMT:playNhsSound", "checkPatientAlarm")
    e.bloodPressure1 = math.random(20, 199)
    e.bloodPressure2 = math.floor(e.bloodPressure1 - e.bloodPressure1 / 2.9)
    if e.bloodPressure2 > e.bloodPressure1 then
    ...
    e.coma = false
    Wait(3000)
    TriggerServerEvent("GMT:playNhsSound", "safeToTouch")
    Wait(12000)
    TriggerServerEvent("GMT:playNhsSound", "threeBeeps")
end)
local B = {
...
local function C()
    if globalNHSOnDuty or globalLFBOnDuty and not GMT.getUserId() == 1 then
        local D = PlayerPedId()
        local E = GetSelectedPedWeapon(D)
        if E ~= `WEAPON_UNARMED` then
            if not table.has(B, E) then
                GMT.setWeapon(D, "WEAPON_UNARMED", true)
            end
        end
    end
end
    ...
    while true do
        if e.active and e.targetPed then
            ...
            if #(F - G) > 10.0 then
                e.active = false
                TriggerServerEvent("GMT:playNhsSound", "threeBeeps")
                GMT.notify("~r~Lifepak disconnected.")
            end
        end
        Citizen.Wait(5000)
    end
...
local H = false
RegisterNetEvent("GMT:attemptCPR",function()
    H = true
    GMT.loadAnimDict("missheistfbi3b_ig8_2")
    ...
    while H do
        if not IsEntityPlayingAnim(PlayerPedId(), "missheistfbi3b_ig8_2", "cpr_loop_paramedic", 3) or IsControlJustReleased(0, 73) then
            H = false
            TriggerServerEvent("GMT:cancelCPRAttempt")
            break
        end
        Citizen.Wait(0)
    end
end)
RegisterNetEvent("GMT:cancelCPRAttempt", function()
    H = false
    if GMT.canAnim() then
        ClearPedTasks(PlayerPedId())
    end
end)
RegisterNetEvent("GMT:cancelCPR", function()
    ReviveReady = true
    if ReviveReady then
        ClearPedTasks(PlayerPedId())
    end
