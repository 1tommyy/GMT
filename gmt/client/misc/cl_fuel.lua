    ...
    function(b, c)
        if c then
            for d, e in pairs(a.stations) do
                tGMT.addBlip(e.x, e.y, e.z, 361, 4, "Petrol Station", 0.6)
            end
        end
    end
)
...
function tGMT.getVehicleFuel(l)
...
local function r(l)
    ...
    elseif not k then
        setVehicleFuel(l, tGMT.getVehicleFuel(l))
        k = true
    end
    ...
    SendNUIMessage({showFuel = true, fuel = tGMT.getVehicleFuel(l)})
end
...
AddEventHandler(
    ...
    function(z, t, l)
        ...
        if z then
            TriggerServerEvent("GMT:takeAmount", math.floor(i), "Fuel charge")
        end
        local F = GetEntityModel(l)
        local G = GMT.getVehicleIdFromHash(F)
        if G then
            local H = GetFuel(l)
            TriggerServerEvent("GMT:updateFuel", G, math.floor(H * 1000) / 1000)
        end
        i = 0.0
    end
)
AddEventHandler(
    ...
    function(z, t, l)
        ...
        GMT.loadAnimDict("timetable@gardener@filling_can")
                    ...
                    elseif f then
                        ...
                        if j >= a.jerryCanCost then
                            if not HasPedGotWeapon(t, 883325847) then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    GMT.allowWeapon("883325847")
                                    GiveWeaponToPed(t, 883325847, 4500, false, true)
                                    TriggerServerEvent("GMT:takeAmount", a.jerryCanCost, "Petrol Can")
                                    j = 50000
                                end
                            else
                                ...
                                if V > 0 then
                                    ...
                                    if IsControlJustReleased(0, 38) then
                                        TriggerServerEvent("GMT:takeAmount", math.floor(V), "Fuel Charge")
