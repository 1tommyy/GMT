...
local function c()
    if GetEntityHealth(GMT.getPlayerPed()) <= 102 then
        GMT.notify("~r~You try to move but you remember you're dead")
        return true
    elseif IsEntityDead(GMT.getPlayerPed()) then
        GMT.notify("~r~You try to move but you remember you're dead")
        return true
    else
        return false
...
RegisterCommand("bag",function(f, g)
    ...
    if DoesEntityExist(h) and GMT.canAnim() and not IsEntityDead(h) and d() then
...
function tGMT.FreezePlayerControls(variable)
...
function tGMT.globalSurrenderring(playerSrc)
    if GMT.isPlayerSurrendered() then
        return true
    else
        return false
    end
...
function surrender()
    local h = PlayerPedId()
    if DoesEntityExist(h) and GMT.canAnim() and not IsEntityDead(h) and (d() or isPlayerNearPrison()) then
        GMT.loadAnimDict("random@arrests")
        GMT.loadAnimDict("random@arrests@busted")
...
function GMT.isPlayerSurrendered()
...
function GMT.isPlayerSurrenderedNoProgressBar()
    return j()
end
local u
RegisterNetEvent(
    "GMT:startRobbingPlayer",
    function()
        u = GetGameTimer()
    end
)
RegisterNetEvent(
    "GMT:endRobbingPlayer",
    function()
        u = nil
    end
)
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if B then
                ...
                GMT.notify("Press ~r~[E]~w~ to take a picture!")
                B = true
            end
        end
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if y then
                ...
                Wait(120)
                GMT.notify("Press ~r~[E]~w~ to take a toke!")
                y = true
            end
        end
        while y do
            ...
            if IsControlJustPressed(0, 38) then
                ...
                Wait(100)
                GMT.notify("You take a huge rip!")
                Wait(7250)
            end
        end
    end,
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if v then
                ...
                Wait(120)
                GMT.notify("Press ~r~[E]~w~ to wave the wand.")
                v = true
            end
        end
            ...
            if
             ...
             then
                if not Y and globalNHSOnDuty then
                    ...
                    Y = false
                    DetachEntity(GMT.getPlayerPed(), false, false)
                    ClearPedSecondaryTask(GMT.getPlayerPed())
                    local a5 = GetClosestPlayer(3)
                    local a7 = GetPlayerServerId(a5)
                    if a7 ~= 0 then
                        TriggerServerEvent("GMT:animationSyncStop", a7)
                    end
                end
                Wait(250)
            end
...
RegisterNetEvent(
    ...
    function()
        ...
        SetPedMotionBlur(GMT.getPlayerPed(), true)
        SetPedMovementClipset(GMT.getPlayerPed(), "move_m@drunk@verydrunk", true)
        RemoveAnimSet("move_m@drunk@verydrunk")
        SetPedIsDrunk(GMT.getPlayerPed(), true)
        DoScreenFadeIn(1000)
        ClearPedSecondaryTask(GMT.getPlayerPed())
        DeleteObject(prop)
        Wait(60000)
        ...
        ResetPedMovementClipset(GMT.getPlayerPed(), 0)
        SetPedIsDrunk(GMT.getPlayerPed(), false)
        SetPedMotionBlur(GMT.getPlayerPed(), false)
        RemoveAnimDict(H)
    end
)
RegisterNetEvent(
...
RegisterNetEvent(
    ...
    function(ai)
        ...
        GMT.loadAnimDict("misshair_shop@hair_dressers")
        tGMT.setCanAnim(false)
        local e = PlayerPedId()
        ...
        GMT.loadModel("prop_clippers_01")
