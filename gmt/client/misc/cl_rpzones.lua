...
local function f()
    ...
    g.position = GMT.getPlayerCoords()
    g.radius = g.radius + 0.0
    g.vehicles.speed = g.vehicles.speed + 0.0
    TriggerServerEvent("GMT:createRPZone", g)
end
...
local function k()
    ...
    if l then
        TriggerServerEvent("GMT:removeRPZone", l)
    end
end
local p = false
RMenu.Add(
    "rpzones",
    "mainmenu",
    RageUI.CreateMenu("", "~s~M~w~ain Menu", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight(), "menus", "adminmenu")
)
RMenu.Add(
    "rpzones",
    "weapons",
...
local function w(g, x)
    ...
    if not g.weapons.melee and y == `GROUP_MELEE` then
        GMT.setWeapon(j, "WEAPON_UNARMED")
...
local function I(g)
    local n, x = GetCurrentPedWeapon(GMT.getPlayerPed())
    if x ~= 0 and x ~= `WEAPON_STAFFGUN` then
        w(g, x)
    end
    local A, J = GMT.getPlayerVehicle()
    if A ~= 0 and J then
        C(g, A)
    end
...
GMT.createThreadOnTick(K)
function GMT.GetRPZoneInfo()
    local M = GMT.getPlayerCoords()
    for n, g in ipairs(d) do
        if #(M - g.position) < g.radius then
            return g
