        ...
        ontick_golfMarker = function()
            ...
            if IsControlJustPressed(1, 38) then
                if d then
                    ...
                    TriggerServerEvent("GMT:takeGolfMoney")
                end
            end
        end
        GMT.createArea("golf_course", M, 15, 6, enter_golfMarker, exit_golfMarker, ontick_golfMarker, {})
    end
)
local function N(O, P, Q, R, S, T, U, V)
    ...
    attachModel = O
    boneNumber = P
    SetCurrentPedWeapon(GMT.getPlayerPed(), 0xA2719263, true)
    local W = GetPedBoneIndex(GMT.getPlayerPed(), P)
    GMT.loadModel(attachModel)
    attachedProp = CreateObject(attachModel, 1.0, 1.0, 1.0, true, true, false)
    AttachEntityToEntity(attachedProp, GMT.getPlayerPed(), W, Q, R, S, T, U, V, true, true, false, false, 2, true)
end
local function X(Y)
...
RegisterNetEvent(
    ...
    function()
        ...
        r = GMT.spawnVehicle("caddy", -1332.7823486328, 128.18229675293, 56.032329559326, 180, true, true, false)
        local Z = AddBlipForEntity(r)
        SetBlipSprite(Z, 326)
        SetBlipColour(Z, 5)
        d = true
        SetCurrentPedWeapon(GMT.getPlayerPed(), 0xA2719263, false)
        X("golfbag01")
        if GetResourceKvpInt("gmt_golf_done_cutscene") == 0 then
            D()
            SetResourceKvpInt("gmt_golf_done_cutscene", 1)
        end
    end
)
local function _()
    if d and a ~= 0 and not i then
        ...
        local A =
            ...
            b ..
                "~r~ | ~s~" ..
                    c .. "~r~ | ~s~" .. q .. "~r~ | ~s~" .. math.ceil(#(GMT.getPlayerCoords() - y[a].endHole)) .. " m"
        GMT.DrawText(0.448, 0.91, A, 0.6, 4)
    end
end
GMT.createThreadOnTick(_)
local function a0(Q, R, S)
    DeleteObject(k)
    k = CreateObject("prop_golf_ball", Q, R, S, true, true, false)
    SetEntityRecordsCollisions(k, true)
    tGMT.removeBlip(m)
    m = tGMT.addBlip(Q, R, S, 161, 0, "Golf Ball", 0.6)
    ...
    local a1 = GetEntityHeading(GMT.getPlayerPed())
...
local function ae(af)
    GMT.loadAnimDict("mini@golf")
    if not IsEntityPlayingAnim(PlayerPedId(), "mini@golf", af, 3) then
        length = GetAnimDuration("mini@golf", af)
        TaskPlayAnim(GMT.getPlayerPed(), "mini@golf", af, 1.0, -1.0, length, 0, 1, false, false, false)
        Citizen.Wait(length)
    end
end
local function ag()
...
local function ao()
    ...
    PlaySoundFromEntity(-1, "GOLF_SWING_FAIRWAY_IRON_LIGHT_MASTER", GMT.getPlayerPed(), 0, nil, 0)
    ae(playAnim)
    ak()
    Citizen.Wait(1000)
    a9()
            ...
            if d then
                if e == 1 then
                    ...
                    if a == 10 then
                        ...
                        SetBlipDisplay(p, 2)
                        n = tGMT.addBlip(y[a].endHole.x, y[a].endHole.y, y[a].endHole.z, 109, 1, "Hole", 1.0)
                    ...
                    elseif f == 1 and not i and not l then
                        if GMT.getPlayerVehicle() == 0 then
                            local aa = GetEntityCoords(k)
                            if #(GMT.getPlayerCoords() - aa) > 50 and not s then
                                if GetGameTimer() - t < 15000 then
                                    ...
                                    if IsControlJustReleased(1, 38) then
                                        ...
                                        while l do
                                            if
                                                (#(GMT.getPlayerCoords() - aa) < 100.0 or
                                                    GMT.getPlayerCoords().z - aa.z > 100.0) and
                                                    #(GMT.getPlayerCoords() - y[a].endHole) > 50.0
                                             then
                                                z("Press ~g~E~s~ to drop here.", false, -1)
                                                if IsControlJustReleased(1, 38) then
                                                    l = false
                                                    local a6 = GMT.getPlayerCoords()
                                                    a0(a6.x, a6.y, a6.z - 1)
                                                    b = b + 1
                                                end
                                            else
                            ...
                            elseif #(GMT.getPlayerCoords() - aa) < 2 and not l then
                                f = 2
                                e = 0
                            end
                        end
...
AddEventHandler(
    ...
    function(au, av)
        if av then
            tGMT.addBlip(-1350.1905517578, 134.2918548584, 55.556828308105, 109, 68, "Golf Course", 1.0)
        end
    end
)
