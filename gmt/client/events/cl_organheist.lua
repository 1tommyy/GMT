...
AddEventHandler(
    ...
    function(p, q)
        if q then
            tGMT.addMarker(b.x, b.y, b.z, 0.7, 0.7, 0.5, 0, 125, 255, 125, 150, 20, false, false, true)
            tGMT.addBlip(b.x, b.y, b, 378, 3, "Organ Heist Police")
            tGMT.addMarker(c.x, c.y, c.z, 0.7, 0.7, 0.5, 255, 0, 0, 125, 150, 20, false, false, true)
            tGMT.addBlip(c.x, c.y, c.z, 378, 1, "Organ Heist Criminals")
        end
    end
)
    ...
    function()
        while true do
            if f then
                ...
                if IsControlJustPressed(0, 38) then
                    if not globalOnPoliceDuty then
                        if not g then
                            TriggerServerEvent("GMT:joinOrganHeist")
                            Wait(100)
                        else
                            GMT.notify("~r~Already joined the Organ Heist!")
                        end
                    else
                        GMT.notify("~r~You are police, please use the other entrance!")
                    end
                end
            elseif e then
                ...
                if IsControlJustPressed(0, 38) then
                    if globalOnPoliceDuty then
                        ...
                        GMT.notify("~r~You are a civillian, please use the other entrance!")
                    end
                end
            end
            ...
            if inGamePhase or inWaitingStage then
                ...
                RemoveWeaponFromPed(GMT.getPlayerPed(), "WEAPON_MOLOTOV")
                RemoveWeaponFromPed(GMT.getPlayerPed(), "WEAPON_FLASHBANG")
                RemoveWeaponFromPed(GMT.getPlayerPed(), "WEAPON_SMOKEGRENADE")
                RemoveWeaponFromPed(GMT.getPlayerPed(), "WEAPON_SMOKEGRENADEGMT")
                RemoveWeaponFromPed(GMT.getPlayerPed(), "WEAPON_SMOKEGRENADEGMTPD")
                RemoveWeaponFromPed(GMT.getPlayerPed(), "WEAPON_BZGAS")
                RemoveWeaponFromPed(GMT.getPlayerPed(), "WEAPON_STUNGUN")
                local r = PlayerPedId()
...
RegisterNetEvent(
    "GMT:organHeistKillConfirmed",
    function(B)
        PlaySoundFrontend(-1, "Weapon_Upgrade", "DLC_GR_Weapon_Upgrade_Soundset", true)
        tGMT.playScreenEffect("MP_Celeb_Win", 0.25)
        GMT.notify("~g~Killed " .. B .. " received £50,000 and 25% Armour")
    end
)
Citizen.CreateThread(
    function()
        while true do
            if inWaitingStage then
                ...
                if #(s - k.pastGate) < 3.0 then
                    ...
                    NetworkFadeInEntity(PlayerPedId(), 0)
                    DoScreenFadeIn(1000)
                    GMT.notify("~r~You got too far from the organ heist and have been teleported back.")
                end
            end
            if inGamePhase then
                local s = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent("GMT:startOrganHeist",function()
...
RegisterNetEvent(
    ...
    function()
        ...
        inGamePhase = false
        d = 600
        GMT.deleteAtm("Organ Heist")
        for l, z in pairs(k.gunStores[h]) do
            tGMT.deleteGunStore(z[1])
        end
        h = ""
        k = nil
        ...
        TriggerServerEvent("GMT:ForceStoreAllWeapons")
        Wait(10000)
...
function GMT.setDeathInOrganHeist()
    inOrganHeist = false
    inWaitingStage = false
    inGamePhase = false
    WaitingForOrganToBegin = false
end
RegisterNetEvent("GMT:ReadyForOrgan", function()
    WaitingForOrganToBegin = true
    if not inOrganHeist then
       GMT.notify("~g~Organ Heist Starting soon, Use /tporgan")
    end
end)
RegisterCommand("tporgan", function()
    local source = source
    local user_id = GMT.getUserId(source)
    local name = GMT.getPlayerName(GetPlayerServerId(PlayerId()))
    local success = false
    local message = ""
    if GetPlayerInvincible(PlayerId()) and isInGreenzone and WaitingForOrganToBegin and not GMT.isInPurge() and not inOrganHeist and not tGMT.isStaffedOn() and not noclipActive and not tGMT.isInComa() and not GMT.isInSpectate() and not GMT.getInRPZone() then
        success = true
        if GMT.globalOnPoliceDuty() then
            GMT.notify("~g~Teleporting to Organ Heist, Police Side.")
            Wait(10000)
            tGMT.teleport(238.61906433105, -1382.0168457031, 33.743103027344)
            message = "Teleported - Police Side"
        else
            GMT.notify("~g~Teleporting to Organ Heist, Criminal Side.")
            Wait(10000)
            tGMT.teleport(224.66180419922, -1368.0705566406, 30.564836502075)
            message = "Teleported - Criminal Side"
        end
        GMT.notify("~b~You are at the Organ Heist. Quick because it starts soon!")
    else
        if not WaitingForOrganToBegin then
            message = "Organ isn't starting"
            GMT.notify("~r~Organ Heist isn't starting anytime soon.")
        elseif inOrganHeist then
            message = "They are in organ"
            GMT.notify("~r~You are currently playing the Organ Heist.")
        elseif not isInGreenzone then
            message = "Not inside greenzone"
            GMT.notify("~r~You must be inside a greenzone to teleport.")
        elseif tGMT.isStaffedOn() then
            message = "User is staffon'd"
            GMT.notify("~r~You cannot be staffed on.")
        elseif noclipActive then
            message = "User is no-clipped"
            GMT.notify("~r~You cannot be in no-clip.")
        elseif tGMT.isInComa() then
            message = "User is dead"
            GMT.notify("~r~Cannot teleport while dead.")
        elseif GMT.isInSpectate() then
            message = "User is spectating"
            GMT.notify("~r~You cannot teleport while spectating.")
        elseif GMT.isInPurge() then
            message = "Purge is active"
            GMT.notify("~r~You cannot teleport to the Organ Hesit while in the purge.")
        elseif GMT.getInRPZone() then
            message = "User is in a rp zone"
            GMT.notify("~r~You cannot be in a roleplay zone.")
        end
    end
    if success then
        TriggerServerEvent("GMT:sendWebhookClient", "organ-tp", "GMT Teleport Organ Logs", string.format("**%s**\n\n> Player Name %s\n> Player PermID: %s", message, name, user_id))
    else
        TriggerServerEvent("GMT:sendWebhookClient", "organ-tp", "GMT Teleport Organ Unsuccessful", string.format("**%s**\n\n> Player Name %s\n> Player PermID: %s", message, name, user_id))
    end
end)