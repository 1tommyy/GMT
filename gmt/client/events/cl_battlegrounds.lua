...
function DoAnim() 
    ...
    GMT.loadAnimDict("anim@gangops@facility@servers@bodysearch@")
...
function GMT.changeGasRadius(M)
    SendNUIMessage({transactionType = "br-gas"})
    currentroyale.gas.timeUntilNext = {minutes = 1, seconds = 0}
    tGMT.announceMpSmallMsg("GAS MOVING", "The gas is closing in!",6,5000)
    Wait(4000)
...
function tGMT.ClearWeapons()
    RemoveAllPedWeapons(PlayerPedId())
end

function GMT.GiveWeaponToPed(weaponHash, ammoCount, isHidden, equipNow)
    ...
    elseif HasPedGotWeapon(Ped, weaponHash, false) then
        ...
        GMT.notify("~g~You already have this weapon refilling ammo!")
    end
end

RegisterNetEvent("GMT:startBattlegrounds",function(I)
    local Ped = PlayerPedId()
    locationID = I
    GMT.startGas(cfg.locations[locationID].gas.initalRadius, cfg.locations[locationID].gas.centre)
    DoScreenFadeOut(1500)
    Wait(1500)
    CurrentEvent.eventName = "Battle Royale"
    TriggerEvent("chat:clear")
    GMT.stopEventSequence(true)
    tGMT.ClearWeapons()
    GMT.GiveWeaponToPed("GADGET_PARACHUTE", 2, false, false)
    tGMT.giveWeapons({["GADGET_PARACHUTE"] = {2}},false,decorpasskey)
    ...
    local J = GMT.spawnVehicle("jet", cfg.locations[locationID].planeStart, cfg.locations[locationID].planeHeading, false, false, true)
    SetModelAsNoLongerNeeded("jet")
    SetEntityCollision(J, false, false)
    SetVehicleEngineOn(J, true, true, false)
    SetPlaneTurbulenceMultiplier(J, 0.0)
    GMT.loadModel("s_m_m_pilot_02")
    ...
    -- Wait for players to be able to exit the plane
    Wait(4000)
    DoScreenFadeIn(4000)

    -- Countdown timer until players can exit the plane
    Citizen.CreateThread(function()
    ...
    -- Allow players to exit the plane after a delay
    SetTimeout(15000, function()
        if CurrentEvent.isActive then
            currentroyale.player.canExitPlane = true
             setCursor(1) -- Assuming this is a function call in your script
            inGUI = not inGUI

            -- Wait for players to exit and handle collision re-enablement
            SetTimeout(45000, function()
                local L = GMT.getPlayerVehicle()
                if CurrentEvent.isActive and currentroyale.data.plane == L then
                    -- Disable collision with blades immediately upon exit
                    SetEntityNoCollisionEntity(Ped, J, true)

                    -- Set player invincible for a short duration after exit
                    SetPlayerInvincible(PlayerId(), true)
                    local invincibilityDuration = 10000  -- 10 seconds in milliseconds
                    SetTimeout(invincibilityDuration, function()
...
RegisterNetEvent("GMT:BattleGrounds:Cleanup",function()
    CurrentEvent.isActive = false
    for A,B in pairs(currentroyale.data.lootBoxes) do
        tGMT.removeArea(B.areaId)
        tGMT.removeNamedBlip(B.areaId)
...
RegisterNetEvent("GMT:Battlegrounds:Podium",function(winners,losers)
   -- print("[GMT] Battlegrounds Winners:" ..json.encode(winners))
   -- print("[GMT] Battlegrounds Losers:".. json.encode(losers))
    ...
    if not quickExit then
        ...
        if winners and losers then
            GMT.podiumLeaderboard(winners, losers)--, currentEmotes)
        end
    end
end)

RegisterNetEvent("GMT:syncLootboxesTable",function(u)
    ...
    GMT.loadModel("prop_box_ammo03a")
    for v, w in pairs(currentroyale.data.lootBoxes) do
        if #(w.coords - currentroyale.gas.coords) < currentroyale.gas.radius / 2 then
            currentroyale.data.lootBoxes[v].areaId = "gmtbr_lootbox_" .. v
            tGMT.setNamedBlip("gmtbr_lootbox_" .. v, w.coords.x, w.coords.y, w.coords.z, 478, 1, "Lootbox", 0.5)
            local x = function(y)
                if currentroyale.data.lootBoxes[y.box] then
                    ...
                    print(string.format("[GMT] lootbox with ID: %s is nil", y.box))
                end
            end
            local z = function(y)
                if currentroyale.data.lootBoxes[y.box] then
            ...
            local A = function(y)
                if currentroyale.data.lootBoxes[y.box] then
                    if y.distance <= 1.5 then
                        ...
                        GMT.DrawText3D(vector3(F.x, F.y, F.z - 0.5), "Press [E] to pickup lootbox.", 0.2)
                        if IsControlJustPressed(0, 51) then
                            if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
                                GMT.notify("~r~You cannot pickup a lootbox while in a vehicle.")
                                return
                            end
                            DoAnim()
                            Wait(500)
                            TriggerServerEvent("GMT:LootBox", y.box)
                        end
                    end
                end
            end
            GMT.createArea("gmtbr_lootbox_" .. v, w.coords, 200.0, 6, x, z, A, {box = v})
        end
    end
    SetModelAsNoLongerNeeded("prop_box_ammo03a")
    GMT.loadModel("prop_armour_pickup")
    for v, w in pairs(currentroyale.data.armourPlates) do
        if #(w.coords - currentroyale.gas.coords) < currentroyale.gas.radius / 2 then
            tGMT.setNamedBlip("gmtbr_armour_" .. v, w.coords.x, w.coords.y, w.coords.z, 175, 1, "Lootbox", 0.5)
            local B = function(C)
                if currentroyale.data.armourPlates[C.plateId] then
                    ...
                    print(string.format("[GMT Events] body armour with ID of %s is nil in table (onEnter)", C.plateId))
                end
            end
            local D = function(C)
                if currentroyale.data.armourPlates[C.plateId] then
                    ...
                    print(string.format("[GMT Events] body armour with ID of %s is nil in table (onLeave)", C.plateId))
                end
            end
            local E = function(C)
                if currentroyale.data.armourPlates[C.plateId] then
                    if C.distance <= 1.5 then
                        ...
                        GMT.DrawText3D(vector3(F.x, F.y, F.z - 0.5), "Press [E] to pickup armour.", 0.2)
                        if IsControlJustPressed(0, 51) then
                            if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
                                GMT.notify("~r~You cannot pickup a armour plate while in a vehicle.")
                                return
                            end
                            if GetPedArmour(PlayerPedId()) < 100 then
                                ...
                                TriggerServerEvent("GMT:removeArmourPlate", C.plateId)
                                SetPedComponentVariation(PlayerPedId(), 9, 15, 2, 0)
                            else
                                GMT.notify("~r~You already have full armour.")
                            end
                        end
                    end
                end
            end
            GMT.createArea("gmtbr_armour_" .. v, w.coords, 200.0, 6, B, D, E, {plateId = v})
            currentroyale.data.armourPlates[v].areaId = "gmtbr_armour_" .. v
        end
    end
    SetModelAsNoLongerNeeded("prop_armour_pickup")
    local vehicle = GMT.loadModel("sovereign")
    for _,location in pairs(currentroyale.data.vehiclelocations) do
        if #(location.coords - currentroyale.gas.coords) < currentroyale.gas.radius / 2 then
            local veh = GMT.spawnVehicle("sovereign", location.coords, 0, false, false, false)
...
RegisterNetEvent("GMT:removeLootBox",function(G)
    tGMT.removeArea("gmtbr_lootbox_" .. G)
    tGMT.removeNamedBlip("gmtbr_lootbox_" .. G)
...
RegisterNetEvent("GMT:removeArmourPlateCl",function(H)
    tGMT.removeArea("gmtbr_armour_" .. H)
    tGMT.removeNamedBlip("gmtbr_armour_" .. H)
...
RegisterNetEvent("GMT:addBRKill",function(usersrc, username)
...
RegisterNetEvent("GMT:removePlayerFromBR",function(usersrc)
    if currentroyale.players[usersrc] then
...
function func_BRMenuControl()
    if a then
        if CurrentEvent.isActive and CurrentEvent.eventName == "Battle Royale" then
            ...
            DrawAdvancedText(0.595, 0.210, 0.005, 0.0028, 0.7, "GMT BR LEADERBOARD", 255, 255, 255, 255, GMT.getFontId("Akrobat-ExtraBold"), 0)
...
GMT.createThreadOnTick(func_BRMenuControl)

local N = {[1] = "1st", [2] = "2nd", [3] = "3rd"}
local O = {[1] = 4, [2] = 3, [3] = 2}
local P = {[1] = "~HUD_COLOUR_GOLD~", [2] = "~HUD_COLOUR_SILVER~", [3] = "~HUD_COLOUR_BRONZE~"}
GMT.createThreadOnTick(function()
    if CurrentEvent.isActive and CurrentEvent.eventName == "Battle Royale" then
        for h = 1, 3 do
            if currentroyale.data.leaderboard[h] then
                GMT.DrawGTATimerBar(P[h] .. N[h] .. " " .. currentroyale.data.leaderboard[h].name,P[h] .. currentroyale.data.leaderboard[h].kills,O[h] + 1)
            end
        end
        GMT.DrawGTATimerBar("~y~GAS:~w~",string.format("~y~%02d:%02d", currentroyale.gas.timeUntilNext.minutes, currentroyale.gas.timeUntilNext.seconds),2)
    end
end)

function GMT.DrawGTAText(a, b, c, j, z, A)
    SetTextFont(0)
    SetTextScale(j, j)
...
function GMT.DrawGTATimerBar(a0, K, a1, a2, a3, a4)
    local a5 = 0.17
    local a6 = -0.01
    local a7 = 0.038
    local a8 = 0.008
    ...
    GMT.DrawGTAText(a0, ac - a5 + 0.06, ae - a8, a4)
    GMT.DrawGTAText(string.upper(K), ac - a6 + (a3 or 0), ae - 0.0175, a2, true, a5 / 2)
end
        ...
        if currentroyale.gas.isActive then
            ...
            if currentroyale.gas.timeUntilNext.seconds > 0 then
                ...
                if currentroyale.gas.timeUntilNext.seconds == 0 and currentroyale.gas.timeUntilNext.minutes == 0 then
                    GMT.changeGasRadius(currentroyale.gas.radius - 400)
                else
...
Citizen.CreateThread(function()
    while true do
        ...
        if currentroyale.gas.isActive and currentroyale.player.hasJumped and not IsPedInParachuteFreeFall(PlayerPedId()) and GetPedParachuteState(PlayerPedId()) <= 2 and CurrentEvent.isActive then
            sleep = true
            if #(GMT.getPlayerCoords().xy - currentroyale.gas.coords.xy) > currentroyale.gas.radius / 2.0 then
                GMT.DrawNativeText("~r~You are in the gas. Get to the safe zone before you suffocate.")
...
function GMT.DrawAdvancedText(b, c, r, s, t, a, u, v, w, x, k, y)
    SetTextFont(k)
    SetTextScale(t, t)
...
function GMT.DrawAdvancedTextNoOutline(b, c, r, s, t, a, u, v, w, x, k, y)
    SetTextFont(k)
    SetTextScale(t, t)
    SetTextJustification(y)
    SetTextColour(u, v, w, x)
...
local function DrawJumpBox()
    if inplane then
        ...
        if currentroyale.data.timer > 0 then
            GMT.DrawAdvancedText(0.587, 0.934, 0.005, 0.0028, 0.971, tostring(currentroyale.data.timer), 255, 255, 255, 255, 4, 0)
        else
            GMT.DrawAdvancedText(0.587, 0.934, 0.005, 0.0028, 0.971, "JUMP", 255, 255, 255, 255, 4, 0)
        end
        GMT.DrawAdvancedText(0.591, 0.042, 0.005, 0.0028, 0.993, "GMT BATTLEGROUNDS", 255, 255, 255, 255, 6, 0)
    end
end
...
local function firework(coords)
    GMT.loadPtfx("proj_indep_firework")
    GMT.loadPtfx("proj_indep_firework_v2")
    ...
    GMT.loadPtfx("scr_indep_fireworks")
...
function GMT.isPodiumDrawing()
    return drawingPodium
end

local cancelPodium = false

function GMT.callCancelPodium()
    cancelPodium = true
end

function GMT.podiumLeaderboard(winners, losers, currentEmotes)
    if not next(winners) and not next(losers) then
        print("GMT.podiumLeaderboard was called with no winners or losers, this will break things!")
    end
  --  GMT.enablePlayerTags(true, false)
    drawingPodium = true
    GMT.setTime(0,0,0)
   -- GMT.clearBounds()
    RequestIpl("stadium")
    ...
    ExecuteCommand("hideui")
    TriggerEvent("gmt:PlaySound", "celebration_music")
    local textHandles = {}
    ...
    for index, playerInfo in pairs(winners) do
        ...
        --table.insert(textHandles, GMT.add3DTextForCoord(playerInfo.name, winnerPositions[index].x,winnerPositions[index].y,winnerPositions[index].z+2, 25))
    end
    for index, playerInfo in pairs(losers) do
        ...
        --table.insert(textHandles, GMT.add3DTextForCoord(playerInfo.name, loserPositions[index].x,loserPositions[index].y,loserPositions[index].z+2, 25))
    end
    ...
    for _, areaId in pairs(textHandles) do
        tGMT.removeArea("3dtext_"..areaId)
    end
...
function GMT.RegisterDebugCommand(commandName, func)
    CreateThread(function()
...
RegisterCommand("testpodium", function()
    GMT.podiumLeaderboard({},{
        {name="Tayser",source=GetPlayerServerId(PlayerId())}
    },nil)
end, false)

-- GMT.RegisterDebugCommand("testpodium", function()
--     GMT.podiumLeaderboard({},{
--         {name="Tayser",source=GetPlayerServerId(PlayerId())}
--     },nil)
-- end)
        ...
        if CursorInArea(GetArea(0.493, 0.944, 0.14, 0.074)) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                if currentroyale.player.canExitPlane then
                    ...
                    TaskLeaveVehicle(PlayerPedId(), currentroyale.data.plane, 64)
                    ...
                    SetTimeout(5000,function()
                        ...
                        while HasPedGotWeapon(q, "GADGET_PARACHUTE", false) do
                            ...
                            if s > 10.0 and s < 250.0 and IsPedInParachuteFreeFall(q) then
                                GMT.DrawNativeNotification("Press ~INPUT_PARACHUTE_DEPLOY~ to deploy your parachute.")
...
local function func_drawWinnerGUI()
    if currentroyale.player.isInWinnerScreen then
        ...
        DrawAdvancedText(0.582, 0.905, 0.005, 0.0028, 0.96599999999999, "#1 "..GMT.getPlayerName(GetPlayerServerId(PlayerId())), 255, 255, 255, 255, 6, 0)
        if CursorInArea(GetArea(0.092, 0.925, 0.154, 0.096)) then
...
GMT.createThreadOnTick(func_drawWinnerGUI)
GMT.createThreadOnTick(DrawJumpBox)
GMT.createThreadOnTick(func_handleClicks)

Citizen.CreateThread(function()
	while true do
		Citizen.Wait(5)
            if IsPedFalling(PlayerPedId()) and CurrentEvent.isActive then
