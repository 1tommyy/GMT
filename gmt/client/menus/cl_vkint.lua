local a = {
    ...
    [74] = {
        id = 74,
        position = vector3(-2172.92,5143.22,2.82),
        name = "VIP Island",
        destination = {75}
    },
    [75] = {
        id = 75,
        position = vector3(-747.962890625,5807.5263671875,17.620700836182),
        name = "Lodges",
        destination = {74}
    },
    [95] = {
...
RMenu.Add("vkint","mainmenu",RageUI.CreateMenu("~b~T~w~eleporter", "Main Menu", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight()))
local b = nil
local function c(d)
    if GMT.getPlayerCombatTimer() > 0 then
        GMT.notify("You can not teleport whilst you have a combat timer.")
        return
    end
...
AddEventHandler('playerSpawned', function(spawn)
    for f, d in pairs(a) do
        tGMT.addMarker(d.position.x, d.position.y, d.position.z - 1, 1.0, 1.0, 1.0, 255, 255, 255, 170, 50, 27)
        GMT.createArea("vkint_" .. d.id,d.position,2.0,10.0,l,m,function()end,d)
    end
end)