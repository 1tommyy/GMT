...
RMenu.Add("gmtwardrobe","mainmenu",RageUI.CreateMenu("", "", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight(), "menus", "gmt_wardrobeui"))
RMenu:Get("gmtwardrobe", "mainmenu"):SetSubtitle("H~w~OME")
RMenu.Add("gmtwardrobe","listoutfits",RageUI.CreateSubMenu(RMenu:Get("gmtwardrobe", "mainmenu"),"","~s~W~w~ardrobe",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight()))
RMenu.Add("gmtwardrobe","equip",RageUI.CreateSubMenu(RMenu:Get("gmtwardrobe", "listoutfits"),"","~s~W~w~ardrobe",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight()))
    ...
    if RageUI.Visible(RMenu:Get('gmtwardrobe', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Save Outfit", "", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    tGMT.clientPrompt("Enter outfit name:", "", function(outfitName)
                        if outfitName and outfitName ~= '' then
                            if not GMT.isPlayerInAnimalForm() then
                                TriggerServerEvent("GMT:saveWardrobeOutfit", outfitName)
                            else
                                GMT.notify("~r~Cannot save animal in wardrobe.")
                            end
                        else
                            GMT.notify("~r~Invalid outfit name")
                        end
                    end)
                end
            end)
            RageUI.Button("Get Outfit Code","Gets a code for your current outfit which can be shared with other players.",{RightLabel = "→→→"},true,function(d, e, f)
                if f then
                    if tGMT.isPlusClub() or tGMT.isPlatClub() then
                        ...
                        GMT.notify("~y~Available @ store.gmtstudios.uk")
                    end
                end
            end,nil)
        end, function()
