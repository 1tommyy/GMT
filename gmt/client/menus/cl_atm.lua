...
local b = false
RMenu.Add("gmtatm", "mainmenu",RageUI.CreateMenu("", "", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight(), "menus", "atm"))
RMenu:Get("gmtatm", "mainmenu"):SetSubtitle("~b~ATM")
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('gmtatm', 'mainmenu')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Deposit","",{RightLabel = "→→→"},not GMT.IsOnGangFundsButton(),function(c, d, e)
                if e then
                    ...
                    if tonumber(f) then
                        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                            if b then
                                ...
                                TriggerServerEvent("GMT:depositAtm", f)
                            else
                                GMT.notify("~r~Not near ATM.")
                            end
                        else
                            GMT.notify("~r~Get out your vehicle to use the ATM")
                        end
                    else
            ...
            RageUI.ButtonWithStyle("Withdraw","",{RightLabel = "→→→"},not GMT.IsOnGangFundsButton(),function(c, d, e)
                if e then
                    local f = getAtmAmount()
                    if tonumber(f) then
                        if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                            if b then
                                tGMT.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
                                TriggerServerEvent("GMT:withdrawAtm", f)
                            else
                                GMT.notify("~r~Not near ATM.")
                            end
                        else
            ...
            RageUI.ButtonWithStyle("Deposit All","",{RightLabel = "→→→"},not GMT.IsOnGangFundsButton(),function(c, d, e)
                if e then
                    if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if b then
                            tGMT.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
            ...
            RageUI.ButtonWithStyle("Withdraw All","",{RightLabel = "→→→"},not GMT.IsOnGangFundsButton(),function(c, d, e)
                if e then
                    if GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if b then
                            tGMT.playAnim(false, {{"amb@prop_human_atm@male@exit", "exit"}}, false)
...
Citizen.CreateThread(function()
    ...
    for m, n in pairs(a.atms) do
        GMT.createArea("atm_" .. m, n, 1.5, 6, i, k, l, {atmId = m})
...
function GMT.createAtm(t, u)
    ...
    GMT.createArea(v, u, 1.5, 6, i, k, function()
    end)
...
local function z(A)
    ...
    for p = 1, math.random(3, 4) do
        ...
        local E = true
        tGMT.minigameCircularProgressBar({Difficulty = D,Timeout = 25000,
        ...
        if not B then
            ...
            tGMT.startCircularProgressBar("", 2000, nil, function()
            end)
...
RegisterNetEvent("GMT:atmWireCuttingSuccessSync", function(A)
    ...
    GMT.loadPtfx("scr_xs_celebration")
...
RegisterNetEvent("GMT:atmRobberyFakeMoney", function(R, S)
    tGMT.setPlayerCombatTimer(60, false)
    local O = GetGameTimer()
    while true do
        local T = GetGameTimer() - O
        if T >= 10000 then
            TriggerEvent("GMT:setDisplayRedMoney", R + S)
            break
        else
            local U = T / 10000
            TriggerEvent("GMT:setDisplayRedMoney", R + math.floor(S * U))
        end
        Citizen.Wait(0)
    end
end)
RegisterNetEvent("GMT:startAtmWireCutting", function(A)
    ...
    tGMT.setCanAnim(false)
    tGMT.setPlayerCombatTimer(30, false)
    GMT.setWeapon(V, "WEAPON_UNARMED", true)
...
RegisterNetEvent("GMT:atmInkArea", function(A)
    local L = a.robberyAtms[A]
    GMT.loadPtfx("veh_xs_vehicle_mods")
    for p = 1, 10 do
...
local function a7()
    ...
    local a4 = GMT.getPlayerCoords() + GetEntityForwardVector(PlayerPedId()) * 1.0
...
local function a9(j)
    ...
    if select(2, GetCurrentPedWeapon(V, 0, false)) == `WEAPON_CROWBAR` then
        ...
        if IsDisabledControlJustPressed(0, 24) and not _ then
           -- print("GMT:atmRobbery: " .. j.robberyId)
            Citizen.CreateThreadNow(function()
                ...
                GMT.loadAnimDict("melee@small_wpn@streamed_core")
                V = PlayerPedId()
...
Citizen.CreateThread(function()
    for A, L in pairs(a.robberyAtms) do
        GMT.createArea("atmrobbery_" .. A, L.xyz, 1.5, 6, a1, a2, a9, {robberyId = A})
    end
end)
