...
local function r(s)
    ...
    if not s and GetGameTimer() - e < 1500 and not GMT.isDev() then
...
local function a9(G)
    if not GMT.canAnim() or tGMT.isHandcuffed() or GMT.inEvent() then
        return true
    end
    local C = PlayerPedId()
...
local function ac(G)
    ...
    if GetGameTimer() - e < 1500 and not GMT.isDeveloper(GMT.getUserId()) then
        notify("~r~Emotes are being rate limited.")
        return
    end
    r(true)
...
local function al(am)
    ...
    if an ~= 0 then
        k = am
        l = an
        TriggerServerEvent("GMT:sendSharedEmoteRequest", an, am)
    else
...
local function aG(aH, aE)
    if aH and IsControlJustPressed(0, 121) then
        ...
        SetResourceKvp("gmt_favourite_emotes", json.encode(j))
    end
end
RageUI.CreateWhile(
    1.0,
...
RegisterNetEvent(
    "GMT:receiveSharedEmoteRequestSource",
    function()
    ...
    if m then
        ...
        elseif IsControlJustPressed(1, 246) then
            if a9() then
                ...
                TriggerServerEvent("GMT:receiveSharedEmoteRequest", n, a.shared[m][4])
            end
            aT()
        end
    end
...
Citizen.CreateThread(
    function()
        j = json.decode(GetResourceKvpString("gmt_favourite_emotes") or "{}") or {}
        TriggerEvent(
            "chat:addSuggestion",
            "/e",
            "Play an emote",
