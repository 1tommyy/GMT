...
local d = false
local e = ""
RMenu.Add("main","groupselector",RageUI.CreateMenu("", "", GMT.getRageUIMenuWidth(), GMT.getRageUIMenuHeight(),"menus", "licenses"))
RMenu:Get("main", "groupselector"):SetSubtitle("Select a job.")
AddEventHandler("GMT:onClientSpawn",function(D, E)
    if E and not GMT.isInPurge() then
...
RegisterNetEvent("GMT:gotJobSelectors",function(h)
    ...
    for q, r in pairs(c) do
        GMT.createArea("selector_" .. q, r.position, 1.5, 6, i, k, l, {selectorId = q})
...
RegisterNetEvent("GMT:jobSelectorCooldown")
AddEventHandler("GMT:jobSelectorCooldown", function(value)
    enabledJobs = value
end)
