RMenu.Add('GMTclothing','mainMenu',RageUI.CreateMenu("","~s~C~w~lothing Store",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight(),"menus","clothes"))
RMenu.Add('GMTclothing','changePed',RageUI.CreateSubMenu(RMenu:Get('GMTclothing','mainMenu'),"","~s~C~w~hange Gender"))
RMenu.Add('GMTclothing','changeClothing',RageUI.CreateSubMenu(RMenu:Get('GMTclothing','mainMenu'),"","~s~C~w~hange your Clothes"))
RMenu.Add('GMTclothing','clearProps',RageUI.CreateSubMenu(RMenu:Get('GMTclothing','mainMenu'),"","~s~C~w~lear Props"))
...
local r = {
    ...
    vector3(277.69772338867, -1335.5568847656, 24.537786483765),
     -- vector3(-2152.08,5191.95,15.72), -- VIP Island
}
local s=1
local t=false
local u=false
    ...
    while true do 
        ...
        for H=1,#r do 
            if#(G-r[H])<=2.5 and not t then 
                ...
                h=true
                RageUI.Visible(RMenu:Get('GMTclothing','mainMenu'),true)
            elseif s==H then 
                ...
                if not RageUI.Visible(RMenu:Get('GMTclothing','changeClothing'))then 
                    F()
                end 
            end 
        end
...
RageUI.CreateWhile(1.0, true, function()
    if not n then
            ...
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                RageUI.Button('~y~Wardrobe',nil,{RightLabel=">>>"},true,function(L,M,N)
                    if N then 
                        if tGMT.isPlusClub()or tGMT.isPlatClub()then
                            ...
                            GMT.notify("~y~Available @ store.gmtstudios.co.uk")
                        end 
                    end 
                end)
                if PlayerIsInGang then
                    RageUI.Button('Apply Gang Outfit', 'Applys the outfit set by the Gang Leader.', {RightLabel="→→→"}, true, function(L,M,N)
                        if N then
                                TriggerServerEvent("GMT:applyGangFit", gangID)
                            end
                        end)
                    end
                RageUI.Button('Apply Outfit Code','Applys an outfit code given by a player with ~y~GMT Plus~w~ or ~y~GMT Platinum~w~.',{RightLabel=">>>"},true,function(L,M,N)
                    if N then 
                        tGMT.clientPrompt("Outfit Code:","",function(Q)
                            local R = string.gsub(Q, "%s+", "")
                            if #R == 5 then
                                TriggerServerEvent("GMT:applyOutfitCode", string.upper(R))
                            else
                ...
                end,RMenu:Get('GMTclothing','clearProps'))
            end)
        end
        if RageUI.Visible(RMenu:Get('GMTclothing', 'changePed')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                RageUI.Button('MP Male','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            GMT.loadCustomisationPreset("DefaultMale")
                        else
                            GMT.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
                end,function()
                end)
                RageUI.Button('MP Female','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            GMT.loadCustomisationPreset("DefaultFemale")
                        else
                            GMT.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
        ...
        if RageUI.Visible(RMenu:Get('GMTclothing', 'changeClothing')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                m=true
                if h then 
                    for H=1,#a,1 do 
                        ...
                        RageUI.List(P.name,get_drawables(P.type,P.index),a[H].currentListIndex,"Current Texture Index: "..k.textureN.."/"..j-1,{},true,function(L,M,N,Q)
                            ...
                            if N then 
                                ...
                                if k.index==2 then 
                                    ...
                                    TriggerServerEvent("GMT:saveClothingHairData",GetPedDrawableVariation(GMT.getPlayerPed(),2),GetPedHairColor(GMT.getPlayerPed()))
...
function current_clothing(R,i,S)
    if R=="prop"then 
        if S then 
            if GetPedPropTextureIndex(GMT.getPlayerPed(),i)~=nil then 
                return GetPedPropTextureIndex(GMT.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedPropIndex(GMT.getPlayerPed(),i)~=nil then 
                if GetPedPropIndex(GMT.getPlayerPed(),i)==-1 then 
                    return GetPedPropIndex(GMT.getPlayerPed(),i)+2 
                else 
                    return GetPedPropIndex(GMT.getPlayerPed(),i)+1 
                end 
            else 
                return 0 
            end 
    ...
    elseif R=="drawable"then 
        ...
        elseif S then 
            if GetPedTextureVariation(GMT.getPlayerPed(),i)~=nil then 
                return GetPedTextureVariation(GMT.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedDrawableVariation(GMT.getPlayerPed(),i)~=nil then 
                return GetPedDrawableVariation(GMT.getPlayerPed(),i)+1 
            else 
                return 0 
            end 
        end 
...
function set_clothing(T,U,i,V)
    ...
    if T=="drawable" then 
        ...
        if U==11 and i-1==200 or i-1==398 then
            GMT.notify("~r~You are unable to wear this shirt.")
            return 
        end
...
function get_textures(T,R,i)
    if T=="drawable"then 
        ...
        j=GetNumberOfPedTextureVariations(GMT.getPlayerPed(),R,i-1)
        return j 
    else 
        j=GetNumberOfPedPropTextureVariations(GMT.getPlayerPed(),R,i-1)
        return j 
    end 
end
local function Y()
...
Citizen.CreateThread(function()
    while true do 
        if k.index~=nil and RageUI.Visible(RMenu:Get('GMTclothing','changeClothing'))then 
...
AddEventHandler("GMT:onClientSpawn",function(D, E)
    if E then
		for a7,a8 in pairs(r)do 
            tGMT.addBlip(a8.x,a8.y,a8.z,73,3,"Clothing Store",0.7)
            tGMT.addMarker(a8.x,a8.y,a8.z,0.6,0.6,0.6,10,255,81,170,50,9,false,false,true,"dp_clothing","top",90.0,90.0,0.0)
        end 
	end
end)
