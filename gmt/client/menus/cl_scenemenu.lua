RMenu.Add("gmtscenemenu","main",RageUI.CreateMenu("","GMT Traffic Scene Menu",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight(),"menus","scenemenu"))
RMenu.Add("gmtscenemenu","objects",RageUI.CreateSubMenu(RMenu:Get("gmtscenemenu", "main"),"","~s~S~w~pawn Objects",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight(),"menus","scenemenu"))
RMenu.Add("gmtscenemenu","speedZone",RageUI.CreateSubMenu(RMenu:Get("gmtscenemenu", "main"),"","~s~S~w~et a speed zone",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight(),"menus","scenemenu"))
RMenu.Add("gmtscenemenu","spikes",RageUI.CreateSubMenu(RMenu:Get("gmtscenemenu", "main"),"","~s~S~w~et a spike trap",GMT.getRageUIMenuWidth(),GMT.getRageUIMenuHeight(),"menus","scenemenu"))
local b = {}
local c = {}
    ...
    if RageUI.Visible(RMenu:Get('gmtscenemenu', 'speedZone')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            ...
            RageUI.ButtonWithStyle("Create Speedzone",nil,{},true,function(K, L, M)
                if M then
                    if p == 0 then
                        GMT.notify("~r~Please set a radius")
                    elseif q == 0 then
                        GMT.notify("~r~Please set a speed")
                    else
                        TriggerServerEvent("GMT:createSpeedZone", GMT.getPlayerCoords(), q + 0.0, p + 0.0)
                        GMT.notify("~r~Speed zone created")
                    end
                end
            end,nil)
            RageUI.ButtonWithStyle("Delete Speedzone",nil,{},true,function(K, L, M)
                if M then
                    local Q = GMT.getPlayerCoords()
                    local R = nil
                    local S = 250.0
                    ...
                    if R then
                        ...
                        GMT.notify("~g~Speed zone removed")
                    end
                end
            end,nil)
        end)
...
RegisterNetEvent("GMT:toggleTrafficMenu",function()
    ...
    elseif globalNHSOnDuty then 
        RMenu:Get("gmtscenemenu", "main"):SetSubtitle("NHS Scene management")
    end
end)
...
function deleteObject()
    for n, o in pairs(k) do
        ...
        local Y, Z, _ = table.unpack(GetEntityCoords(GMT.getPlayerPed(), true))
        if DoesObjectOfTypeExistAtCoords(Y, Z, _, 0.9, X, true) then
            ...
            if NetworkGetEntityIsNetworked(a0) and not NetworkHasControlOfEntity(a0) then
                local a1 = GMT.getNetId(a0, "deleteObject()")
                if a1 ~= 0 then
...
function placeSpike()
    local a4 = GMT.getPlayerVehicle()
    if a4 == 0 then
        local a5 = "P_ld_stinger_s"
        local a6 = GMT.getPlayerPed()
...
function deleteSpike()
    local Q = GetEntityCoords(GMT.getPlayerPed())
...
Citizen.CreateThread(function()
    while true do
        local a4, ah = GMT.getPlayerVehicle()
        if a4 ~= 0 and ah then
            local ai = false
            local aj = 1000.0
