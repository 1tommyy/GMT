...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        local h = GMT.getPlayerId()
        local i = GMT.getPlayerVehicle()
        if i == 0 then
            ...
            if not f and GetUsingseethrough() and not GMT.isPlayerInPoliceHeli() and not tGMT.isPlayerInDrone() then
                TriggerServerEvent("GMT:ACBan",3)
                f = true
            end
        end
...
AddEventHandler(
    ...
    function(n)
        ...
        TriggerServerEvent("GMT:ACBan",2)
        l = true
        n(nil)
    end
)
...
for p, q in ipairs(o) do
    AddEventHandler(q,function()
        ...
        TriggerServerEvent("GMT:ACBan",4,"Event: "..q)
        m = true
    end)
end
Citizen.CreateThread(
    function()
        while true do
            Citizen.Wait(1000)
            local h = GMT.getPlayerId()
            local i = GetVehiclePedIsIn(GMT.getPlayerPed(), false)
            ...
            if r > 1.0 then
                TriggerServerEvent("GMT:ACBan",5,"PlayerWeaponDamageModifier: "..r)
                return
            end
            if s > 1.0 then
                TriggerServerEvent("GMT:ACBan",5,"PlayerWeaponDefenseModifier: "..s)
                return
            end
            if t > 1.0 then
                TriggerServerEvent("GMT:ACBan",5,"PlayerWeaponDefenseModifier_2: "..t)
                return
            end
            if u > 1.0 then
                TriggerServerEvent("GMT:ACBan",5,"PlayerVehicleDamageModifier: "..u)
                return
            end
            if v > 1.0 then
                TriggerServerEvent("GMT:ACBan",5,"PlayerVehicleDefenseModifier: "..v)
                return
            end
            if y > 1.0 then
                TriggerServerEvent("GMT:ACBan",5,"GetWeaponDamageModifier: "..y)
                return
            end
            if z > 1.0 then
                TriggerServerEvent("GMT:ACBan",5,"GetPlayerMeleeWeaponDamageModifier: "..z)
                return
            end
...
function GMT.isPlayerAboveGround()
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            N = O
            if
                P > 0.4 and not IsPedFalling(g) and GMT.getStaffLevel() < 2 and not IsPedInParachuteFreeFall(g) and
                    not carryingBackInProgress and
                    not piggyBackInProgress and
                    not GMT.takeHostageInProgress() and
                    GetPedParachuteState(g) <= 0 and
                    not IsPedRagdoll(g) and
                    not IsPedRunning(g) and
                    not GMT.isPlayerRappeling() and
                    not GMT.isPlayerAboveGround() and
                    not tGMT.isPlayerHidingInBoot() and
                    not GMT.isSpectatingEvent() and
                    not noclipActive
             ...
             then
                if not IsPedInAnyVehicle(g, 1) then
                    D = D + 1
                    if D > 100 then
                        TriggerServerEvent("GMT:ACBan",69)
                        D = 0
                    end
                end
            end
            local d, Q = GMT.getPlayerVehicle()
            if DoesEntityExist(d) and Q and P > 0.2 and GMT.getStaffLevel() < 2 then
                if F ~= d then
                    E = 0
                    F = d
                end
                ...
                if J < 5.0 and R < 2.5 then
                    ...
                    if E > 100 and GetGameTimer() - G > 4000 then
                        TriggerServerEvent("GMT:ACBan",6)
                        E = 0
                        G = GetGameTimer()
                    end
                end
            end
            if NetworkIsInSpectatorMode() and not tGMT.inSpectate() then
                TriggerServerEvent("GMT:ACBan",7)
                return
            end
            if GetCamFov(GetRenderingCam()) == 50.0 and not GMT.isInSpectate() and not GMT.isInHeist() and GMT.getStaffLevel() == 0 then
                --TriggerServerEvent("GMT:ACBan",8)
                Wait(60000)
            end
            Wait(0)
        end
...
Citizen.CreateThread(function()
    while true do
        if GetLabelText("notification_buffer") ~= "NULL" then
            TriggerServerEvent("GMT:ACBan",9)
        end
        ...
        while true do
            ...
            for S, T in ipairs(WeaponBL) do
                ...
                if HasPedGotWeapon(PlayerPedId(), GetHashKey(T), false) == 1 then
                    ...
                    TriggerServerEvent("GMT:ACBan",1,"Blacklisted Weapon: "..T)
                    return
                end
            end
            local S, r = StatGetInt(`mp0_shooting_ability`, true)
            if r > 100 then
                TriggerServerEvent("GMT:ACBan",10)
                return
            end
        end
    end
    ...
    while true do
        ...
        for S, T in ipairs(PDWeaponBL) do
            ...
            if HasPedGotWeapon(PlayerPedId(), GetHashKey(T), false) == 1 and globalOnPoliceDuty then
                RemoveAllPedWeapons(PlayerPedId(), false)
                GMT.notify("~r~You cannot equip that weapon while clocked on.")
               -- return
            end
        end
    end
...
Citizen.CreateThread(
    function()
        ...
        local S = 0
        while true do
            if S >= 100 and not tGMT.isInComa() then
                TriggerServerEvent("GMT:ACBan",11)
                return
            end
            if not tGMT.isStaffedOn() and GetEntityHealth(PlayerPedId()) > 102 and not spawning and not GMT.isPedScriptGuidChanging()
             then
                local h = PlayerId()
                local g = PlayerPedId()
                ...
                if g ~= 0 then
                    ...
                    tGMT.setHealth(GetEntityHealth(g) + 2)
                end
            else
                Citizen.Wait(1000)
            end
        ...
        while true do
            ...
            if IsPedShooting(f) and not table.has(V, i) then
                ...
                if R == GetMaxAmmoInClip(f, i) then
                    TriggerServerEvent("GMT:ACBan",5,"Infinite Ammo: "..R)
                    Wait(60000)
                end
            end
            Wait(0)
...
local function at()
    ...
    local ax = #(aw - Y)
    if
        ax < 50.0 or av or carryingBackInProgress or piggyBackInProgress or tGMT.isPlayerHidingInBoot() or
    ...
    if aq(aB, 100.0) then
        if au - a2 > 5000 then
            TriggerServerEvent("GMT:sendVelocityLimit", aw, Y)
            a2 = au
        end
    end
end
RegisterNetEvent(
    "GMT:settingPlayerIntoVehicle",
    function()
        a1 = GetGameTimer()
    end
)
...
local function a_()
    ...
    if aW(b0, aC) or aW(b1, aD) or aW(b2, aE) or aW(b3, aF) then
        ...
        if GetGameTimer() - aG > 1000 and d == aH and au - aI > 5000 and GetEntityHealth(PlayerPedId()) > 102 and au - globalLastSpawnedVehicleTime > 5000 then
            local aQ = aP(d)
            local b4 = GMT.getVehicleIdFromHash(GetEntityModel(d)) or "N/A"
            TriggerServerEvent("GMT:sendVehicleStats", b0, aC, b1, aD, b2, aE, b3, aF, aQ, b4)
            aI = au
        end
    end
    aC = b0
...
--                 TriggerServerEvent("GMT:acType12", globalVehicleModelHashMapping[b6])
--                 return
--             end
--         end
--         Wait(1000)
...
--         TriggerServerEvent("GMT:ACBan",25,"Resource Name: " ..resourcename)
--     end
-- end)

-- AddEventHandler("onClientResourceStop",function(resourcename)
--     TriggerServerEvent("GMT:ACBan",26,"Resource Name: " ..resourcename)
-- end)
...
Citizen.CreateThread(function()
    ...
    while true do
        if
            GetPlayerInvincible(PlayerId()) and not isInGreenzone and
            not tGMT.isStaffedOn() and not noclipActive and
            GMT.getStaffLevel() < 3 and
            not tGMT.isInComa() and
            not GMT.isInSpectate() and
            not GMT.getInRPZone() and
            not GMT.isInPurge() 
        then
        TriggerServerEvent("GMT:ACBan", 12)
            print("GMT:ACBan 13 - Invincible")
            Citizen.Wait(60000)
        end    
        
        if
            GetPlayerInvincible_2(PlayerId()) and not isInGreenzone and
            not tGMT.isStaffedOn() and not noclipActive and
            GMT.getStaffLevel() < 3 and
            not tGMT.isInComa() and
            not GMT.isInSpectate() and
        ...
        if
            not IsEntityVisible(PlayerPedId()) and not noclipActive and
            not GMT.isInSpectate() and
            not GMT.inEvent() and
            not tGMT.isPlayerInDrone() and
          not GMT.isSpectatingEvent() and
            not GMT.isInPurge() and
            not spawning and
            GMT.getStaffLevel() < 3
        then
             TriggerServerEvent("GMT:ACBan", 13)
            print("GMT:ACBan 13 - Invisible")
            Citizen.Wait(60000)
        end
...
RegisterNUICallback("NuiDevTool",function()
    TriggerServerEvent("GMT:ACBan",14)
end)
print("[GMT] - Anti-Cheat initialised- credit to palmer")