...
function GMT.getDrawableTextures(d, g)
    local e, f = b(d)
    if e then
    ...
    if i then
        ...
        if q then
            if r ~= q or m then
                ...
                Wait(0)
                tGMT.giveWeapons(s, true,decorpasskey)
              ...
              --  print("[GMT] Same model detected, not changing model.")
            end
...
function GMT.isPedScriptGuidChanging()
    return k or GetGameTimer() - l < 3000
end
function GMT.loadCustomisationPreset(w)
    local x = a.presets[w]
