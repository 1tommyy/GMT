...
function module(rsc, path) -- load a LUA resource file as module
    if path == nil then -- shortcut for gmt, can omit the resource parameter
        path = rsc
        rsc = "gmt"
    end
    local key = rsc .. path
    ...
    if modules[key] then -- cached module
        ...
        if f then
            ...
            if ar[1] then
                table.remove(ar, 1)
                modules[key] = ar
                --print("[gmt] ^2loaded module ^4" .. rsc .. "^0/^5" .. path.. "^0")
                return table.unpack(ar)
            else
                modules[key] = nil
                print("[gmt] error loading module " .. rsc .. "/" .. path .. ":" .. ar[2])
            end
        else
            print("[gmt] error parsing module " .. rsc .. "/" .. path .. ":" .. err)
        end
    end
end
