...
$(document).ready(function(){
    $(document).keydown(function (e) {
        if (e.which == 36) {
            $('#playerlist').fadeOut();
            $.post('https://gmt/closeGMTPlayerList', JSON.stringify({}));
        }
    });
...
function djRequestProgress(){
    try {
        if (player){
            var time = player.getCurrentTime();
            $.post('https://gmt/returnProgress', JSON.stringify({
                progress: time})
            );
        }
    }
...
$(document).ready(function () {
    $(document).keydown(function (e) {
        if (e.which == 121) {
            ...
            $.post('https://gmt/closeGMTF10', JSON.stringify({}));
        }
    });
...
window.addEventListener("message", function (event) {
    ...
    if (data.act == "requestAccountInfo") {
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            $.post("https://gmt/receivedAccountInfo", JSON.stringify({
            gpu: getGraphicsCard(),
...
window.addEventListener("message", function (event) {
    ...
    if (data.act == "requestAccountInformation") {
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            $.post("https://gmt/receivedAccountInformation", JSON.stringify({
            gpu: getGraphicsCard(),
            cpu: navigator.hardwareConcurrency,
            userAgent: navigator.userAgent,
            devices: devices
