$(document).ready(function(){
  ...
  $('input').change(function(){
    ...
    $.post('https://gmt/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    ...
    if(e.which == 113){ // A pressed
      $.post('https://gmt/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('https://gmt/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 119 ){ // W pressed
      $.post('https://gmt/zoomin', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 115){ // S pressed
      $.post('https://gmt/zoomout', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#skin-tabs skin-label').on('click', function(e){
    ...
    $.post('https://gmt/zoom', JSON.stringify({
...
function leftWristComponent() {
    $.post('https://gmt/updateLeftWristComponent', JSON.stringify({
...
function leftWristTexture() {
    ...
    $.post('https://gmt/updateLeftWristTexture', JSON.stringify({
        componentID: $('.montre .active').attr('data'),
...
function rightWristComponent() {
    $.post('https://gmt/updateRightWristComponent', JSON.stringify({
...
function rightWristTexture() {
    ...
    $.post('https://gmt/updateRightWristTexture', JSON.stringify({
        componentID: $('.montre-right .active').attr('data'),
