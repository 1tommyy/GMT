...
async function RegisterVueComponent(component, path) {
    const response = await fetch("nui://gmt/ui/pnc/components/"+path);
...
function RegisterVue() {
    pnc = new Vue({
        ...
        data: {
            ...
            fto: false,
            news: [
                // {id: 1, title: "News", desc: "test 123 test 123", author: "Tayser", date: "07/1/12"}
            ],
            attention_drawn: [],
            fines: fines,
            searchedPersons:[],
        ...
        methods: {
            addMarker() {
                if(!this.doesPlayerHaveWarningMarkerOfType(this.selectedWarningMarkerType)) {
                    $.post('https://gmt/addmarker', JSON.stringify({
            ...
            wipeAllMarkers() {
                $.post('https://gmt/wipeallmarkers', JSON.stringify({
            ...
            addPoint() {
                $.post('https://gmt/addpoint', JSON.stringify({
            ...
            switchPage(page) {
                if(this.personPages.includes(page)){
                    if (this.selectedPerson.id){
                        ...
                        console.log("[GMT PNC] Person needs to be selected to enter this page")
                    }
                }else {
                    this.page = page;
                }
            ...
            closePNC() {
                $.post('https://gmt/exit', JSON.stringify({}));
            },
            searchPerson(){
                let first = this.personFirstName;
                let last = this.personLastName;
                if (first == "" || last == "") {
                    console.log("[GMT PNC] Full name needs to be provided")
                }else{
                    $.post('https://gmt/personsearch', JSON.stringify({
                        firstname: first,
                        lastname: last
                    }));
                }
            ...
            searchPlate(){
                ...
                $.post("https://gmt/platesearch", JSON.stringify({
                    plate: this.plate
                }))
            },
            selectPerson(person){
            ...
            submitFine() {
                ...
                $.post('https://gmt/submitfine', JSON.stringify({
            ...
            addAD(){
                ...
                if (this.ADInput.length === 0) {
                    ...
                    $.post('https://gmt/addattentiondrawn', JSON.stringify({
                        ad: this.ADInput
                    }));
                }
            ...
            removeAD(index, ad) {
                if(this.askedIfSure) {
                    ...
                    $.post('https://gmt/removeattentiondrawn', JSON.stringify({
            ...
            saveNotes() {
                $.post('https://gmt/savenotes', JSON.stringify({
            ...
            savePersonNotes() {
                $.post("https://gmt/savepersonnotes", JSON.stringify({
                    user_id: this.selectedPerson.id,
                    notes: this.selectedPerson.notes
                }))
            },
            GetWarrant() {
                $.post("https://gmt/generatewarrant", JSON.stringify({}))
            }
            },
