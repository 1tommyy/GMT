...
RegisterNetEvent("GMTDEATHUI:setStatistics", function(stats, statsTotal, userId)
    SendNUIMessage({
        type="SET_STATS",
        info = {
            stats = stats,
...
RegisterCommand("openstats", function()
    ...
    if not statsLoaded then
        statsLoading = true
        TriggerServerEvent("GMT:requestStatistics")
...
RegisterNetEvent("GMT:setIsNewPlayer", function()
	bIsNewPlayer = true
end)