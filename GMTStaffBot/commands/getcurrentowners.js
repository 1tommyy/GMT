...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    let count = 0
    fivemexports.gmt.execute("SELECT * FROM `gmt_user_vehicles` WHERE vehicle = ?", [params[0]], (result) => {
        var owners = []
        for (i = 0; i < result.length; i++) { 
            ...
            fivemexports.gmt.execute("SELECT * FROM `gmt_users` WHERE id = ?", [user_id], (result) => {
                ...
                if (count == results){
                    let embed = {
                        ...
                        "color": 0x57f288,
                        "footer": {
                            "text": `GMT Studios | gmtstudios.com`
                        },
                        "timestamp": new Date()
                    }
