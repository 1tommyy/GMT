...
exports.runcmd = async (fivemexports, client, message, params) => {
    ...
    if (!code || code.length === 0) {
        ...
        fivemexports.gmt.execute("SELECT * FROM `gmt_verification` WHERE LOWER(code) = LOWER(?)", [code], (result) => {
            if (result && result.length > 0) {
                ...
                fivemexports.gmt.execute("UPDATE `gmt_verification` SET ? WHERE code = ?", [updateValues, code], async (updateResult) => {
                    if (updateResult) {
                        const embed = new Discord.RichEmbed()
                            .setTitle("Verify")
                        ...
                        await message.member.addRole("1304983610574770268");
                    }
                });
            } else {
                ...
                if (userAttempts + 1 === 3) {
                    const embed = new Discord.RichEmbed()
                        ...
                        .setDescription("When it's your first time connecting to **GMT**, you will be prompted with a box telling you how to verify. \n If you are still struggling, enter the red text into https://discord.com/channels/1265769327055867935/1195851569938321423 \nMake sure you include `!verify`. Here's an example of how it's used")
                        .setImage("https://cdn.discordapp.com/attachments/1195851571150467097/1209909033067089981/GMT_Banner.png?ex=65fb1798&is=65e8a298&hm=472665563bf5fdb4b4ffea3ebb6ada6883328f242a618f6c498067d4338c3f2e&")
                        .setColor(settingsjson.settings.botErrorColour);
