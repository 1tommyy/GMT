...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.gmt.getConnected([parseInt(params[0])], (connected) => {
        userConnected = connected || 'N/A';
    });

    fivemexports.gmt.execute("SELECT * FROM `gmt_user_vehicles` WHERE user_id = ?", [params[0]], (cars) => {
        fivemexports.gmt.execute("SELECT * FROM `gmt_user_data` WHERE user_id = ?", [params[0]], (userdata) => {
            fivemexports.gmt.execute("SELECT * FROM gmt_warnings WHERE user_id = ?", [params[0]], (warnings) => {
                fivemexports.gmt.execute("SELECT * FROM gmt_user_moneys WHERE user_id = ?", [params[0]], (money) => {
                    fivemexports.gmt.execute("SELECT * FROM gmt_casino_chips WHERE user_id = ?", [params[0]], (chips) => {
                        fivemexports.gmt.execute("SELECT * FROM `gmt_users` WHERE id = ?", [params[0]], (users) => {
                            fivemexports.gmt.execute("SELECT discord_id FROM `gmt_verification` WHERE user_id = ?", [params[0]], (discordid) => {
