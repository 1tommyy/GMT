...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.gmt.execute(
        "SELECT user_id FROM `gmt_verification` WHERE discord_id = ?",
        [message.author.id],
        (result) => {
            if (result.length === 0) {
                const embed = {
            ...
            fivemexports.gmt.execute("SELECT * FROM `gmt_stores` WHERE user_id = ?", [user_id], (itemsArray) => {
