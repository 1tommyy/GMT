...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    if (permID === '1' || permID === '2' || permID === '3') {
        let embed = {
            ...
            "thumbnail": {
                "url": "https://cdn.discordapp.com/attachments/1195851571150467097/1209823333349072946/GMT_logo.png?ex=65e852c8&is=65d5ddc8&hm=a969962d0a243906dd80323519699d01a6609b207259b1ee717b4e4be3563b8d&"
            }
        };
        return message.channel.send({ embed });
    }

    fivemexports.gmt.execute("SELECT discord_id FROM `gmt_verification` WHERE user_id = ?", [permID], (result) => {
        if (result.length > 0) {
            ...
            message.channel.send({ embed: confirmEmbed }).then(confirmMessage => {
                ...
                collector.on('collect', async (reaction) => {
                    if (reaction.emoji.name === '✅') {
                        // Ban logic here
                        let newval = fivemexports.gmt.gmt('banDiscord', [permID, 'perm', 'Community Ban', `GMT`]);
