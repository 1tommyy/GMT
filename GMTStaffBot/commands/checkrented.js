...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    let count = 0
    fivemexports.gmt.execute("SELECT * FROM `gmt_user_vehicles` WHERE rentedid = ?", [params[0]], (result) => {
        if (result.length > 0) {
        var rentedvehicles = []
        ...
        for (i = 0; i < results; i++) {
            ...
            fivemexports.gmt.execute("SELECT * FROM `gmt_users` WHERE id = ?", [renter], (result) => {
                a = new Date()
