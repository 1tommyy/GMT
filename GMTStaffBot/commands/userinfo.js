...
exports.runcmd = async(fivemexports, client, message, params) => {
    ...
    fivemexports.gmt.execute("SELECT * FROM `gmt_user_ids` WHERE user_id = ?", [params[0]], (discordid) => {
        ...
        fivemexports.gmt.execute("SELECT * FROM gmt_user_moneys WHERE user_id = ?", [params[0]], (result) => {
            if (result) {
                bank = result[0].bank
                wallet = result[0].wallet
                fivemexports.gmt.execute("SELECT * FROM `gmt_users` WHERE id = ?", [params[0]], (userdata) => {
                    if (userdata) {
                        ...
                        fivemexports.gmt.execute("SELECT * FROM `gmt_users` WHERE id = ?", [params[0]], (result) => {
                            if (result) {
