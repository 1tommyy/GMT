    ...
    fivemexports.gmt.execute("SELECT * FROM cardevs WHERE userid = ?", [message.author.id], (user) => {   
        if (user[0].userid == message.author.id) {
            if (!user[0].currentreport == 0) {
                ...
                fivemexports.gmt.execute("SELECT * FROM cardev WHERE completed = ? AND claimed = ? ORDER BY reportid", [false, false], (report) => {
                    if (report) {
                        fivemexports.gmt.execute("UPDATE cardevs SET currentreport = ? WHERE userid = ?", [report[0].reportid ,message.author.id])
                        fivemexports.gmt.execute("UPDATE cardev SET claimed = ? WHERE reportid = ?", [message.author.id,report[0].reportid])
                        let embed = {
                            "title": "Next Car Report",
            ...
            fivemexports.gmt.execute("INSERT INTO cardevs (userid, reportscompleted, currentreport) VALUES(?, ?, ?)", [message.author.id, 0, 0]);
            fivemexports.gmt.execute("SELECT * FROM cardev WHERE completed = ? AND claimed = ? ORDER BY reportid", [false, false], (report) => {
                if (report) {
                    fivemexports.gmt.execute("UPDATE cardevs SET currentreport = ? WHERE userid = ?", [report[0].reportid ,message.author.id])
                    fivemexports.gmt.execute("UPDATE cardev SET claimed = ? WHERE reportid = ?", [message.author.id,report[0].reportid])
